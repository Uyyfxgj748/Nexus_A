...
const { encodeBooruTags } = require('./utils');
const { verificarGacha } = require('./permisos');
const { resolverJid } = require('./lidResolver');
const { H, SH, F, FS, FI, FE, FC, FA, OK, ERR, WARN, INFO, DIV, barra } = require('./style');
const { buscarPinsDirecto } = require('./pinterest');
const { obtenerCliente: _pixivCliente, descargarImagenPixiv: _pixivDescargar } = require('./pixiv');
...
function barraCooldown(restante, total) {
    return barra(total - restante, total, 10);
}
...
// ── Rolls activos por grupo (en memoria + disco) ─────────────────────────────
...
// ── Persistencia de rolls activos ────────────────────────────────────────────
// Escribe data/rolls_activos.json en cada cambio; se recarga al arrancar.
// Los rolls cuyo rollTime supere ROLL_EXPIRACION se descartan al cargar.
const ROLLS_PATH = path.join(__dirname, '../data/rolls_activos.json');

function _persistirRolls() {
    try {
        const obj = {};
        for (const [gid, roll] of pendingGroupRolls) obj[gid] = roll;
        fs.writeFileSync(ROLLS_PATH, JSON.stringify(obj, null, 2));
    } catch { /* no bloquear el comando si el disco falla */ }
}

// Garantizar flush de rolls pendientes aunque el proceso muera abruptamente (G-01)
process.on('exit',   _persistirRolls);
process.on('SIGTERM', () => { _persistirRolls(); process.exit(0); });

// IIFE: carga los rolls no expirados al iniciar el módulo
(function _cargarRolls() {
    try {
        if (!fs.existsSync(ROLLS_PATH)) return;
        const data = JSON.parse(fs.readFileSync(ROLLS_PATH, 'utf8'));
        const ahora = Date.now();
        let cargados = 0;
        for (const [gid, roll] of Object.entries(data || {})) {
            if (typeof roll.rollTime === 'number' && ahora - roll.rollTime < ROLL_EXPIRACION) {
                pendingGroupRolls.set(gid, roll);
                cargados++;
            }
        }
        if (cargados > 0) console.log(`[GACHA] 🎲 ${cargados} roll(s) activo(s) restaurado(s) desde disco.`);
    } catch { /* JSON corrupto u otro error — empezar vacío */ }
})();
...
function asegurarUsuario(usuarios, jid) {
    if (!usuarios[jid]) {
        usuarios[jid] = { monedas: 100000, harem: [], cooldowns: {}, claimMsg: null, claimMsgGrupo: {}, ventas: [], votosPersonaje: {}, haremGrupo: {}, ventasGrupo: {} };
    }
    ...
    if (!u.claimMsgGrupo) u.claimMsgGrupo = {};
...
// ── Búsqueda en Rule34 (contenido explícito, sin filtro de rating) ──────────
async function _buscarRule34(tagQuery) {
    const headers   = { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' };
    const tagLimpio = tagQuery.trim().replace(/\s+/g, '_');
    try {
        const res = await axios.get(
            `https://api.rule34.xxx/index.php?page=dapi&s=post&q=index&json=1&limit=40&tags=${encodeBooruTags(tagLimpio)}`,
            { timeout: 12000, headers }
        );
        const posts = (Array.isArray(res.data) ? res.data : []).filter(p =>
...
// ── Convierte nombre de serie a tag booru corto ─────────────────────────────
// "Darling in the FranXX" → "darling_in_the_franxx"
// "Re:Zero"               → "re:zero"
// "KonoSuba"              → "konosuba"
function serieATagBooru(serie) {
    return (serie || '')
        .toLowerCase()
        .replace(/[^\w:\s]/g, ' ')   // mantener : para re:zero, sword art: etc.
        .replace(/\s+/g, '_')
        .replace(/_+/g, '_')
        .replace(/^_|_$/g, '')
        .trim();
...
// ── Busca en Safebooru y Danbooru con MÚLTIPLES TAGS separados ──────────────
// Permite hacer "megumin konosuba" como dos tags AND, no como un solo tag
async function buscarPersonajeMultiTag(tagsStr) {
    const headers = { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36' };
    const encoded = encodeBooruTags(tagsStr); // preserva el espacio como separador de tags
    ...
    // Safebooru con nombre+serie
    for (const pid of [0, Math.floor(Math.random() * 5) + 1]) {
        try {
            const res = await axios.get(
                `https://safebooru.org/index.php?page=dapi&s=post&q=index&json=1&limit=40&tags=${encoded}&pid=${pid}`,
                { timeout: 10000, headers }
            );
            const urls = parseSafebooru(res.data);
            if (urls.length) return urls[Math.floor(Math.random() * urls.length)];
    ...
    // Danbooru con nombre+serie (SFW)
    try {
        const res = await axios.get(
            `https://danbooru.donmai.us/posts.json?tags=${encoded}+rating:general&limit=40&random=true`,
            { timeout: 12000, headers }
        );
        const posts = (Array.isArray(res.data) ? res.data : []).filter(p =>
            p.file_url && /\.(jpg|jpeg|png|webp)$/i.test(p.file_url) && !p.is_deleted
        );
        if (posts.length) return posts[Math.floor(Math.random() * posts.length)].large_file_url || posts[Math.floor(Math.random() * posts.length)].file_url;
    } catch { }
...
// ══════════════════════════════════════════════════════════════════════════════
//  SISTEMA DE POOLS PARALELOS
//
//  Cada función "_pool*" recibe un array de variantes de búsqueda ordenadas de
//  más específica a más genérica y retorna el batch de la PRIMERA query que
//  devuelva resultados (cascade). Si ninguna query tiene resultados, retorna [].
//
//  Todas las fuentes activas corren en paralelo; se elige la fuente de mayor
//  prioridad (primera en el array fuentes[]) que devolvió resultados —
//  NO "la que más imágenes tiene" para evitar que fuentes ruidosas ganen.
//
//  Fuentes SFW:  Safebooru, Konachan.com, Danbooru (SFW), Pinterest
//  Fuentes NSFW: Gelbooru, Rule34, Danbooru (sin filtro), Yande.re,
//                Konachan.net, TBIB, Xbooru, HypnoHub, e621
// ══════════════════════════════════════════════════════════════════════════════

const _IMG_RE = /\.(jpg|jpeg|png|webp)(\?.*)?$/i;
const _HDR    = { 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/133.0.0.0 Safari/537.36' };

function _normTag(str) {
    return (str || '').trim().toLowerCase().replace(/\s+/g, '_').replace(/[^\w_:()]/g, '');
}

// Normaliza un query multi-tag (ej: "yor_forger spy_x_family") preservando el
// espacio como separador AND de la API booru. Cada palabra se normaliza por separado.
// Los boorus interpretan los espacios en &tags= como AND entre tags.
// Maneja valores falsy/null/no-string igual que _normTag para evitar throws en pool functions.
function _normMultiTag(q) {
    return String(q || '').trim().split(/\s+/).map(w => _normTag(w)).filter(Boolean).join(' ');
}

function _parseGelbooruPosts(raw) {
    const posts = Array.isArray(raw) ? raw
        : Array.isArray(raw?.post)   ? raw.post
        : Array.isArray(raw?.posts)  ? raw.posts : [];
    return posts.map(p => p.file_url).filter(u => u && _IMG_RE.test(u));
}

function _parseSafebooruPosts(raw) {
    const posts = Array.isArray(raw) ? raw
        : Array.isArray(raw?.post)   ? raw.post
        : Array.isArray(raw?.posts)  ? raw.posts : [];
    return posts.map(p => p.file_url || (p.directory && p.image
        ? `https://safebooru.org//images/${p.directory}/${p.image}` : null))
        .filter(u => u && _IMG_RE.test(u));
}

function _parseMoebooruPosts(raw) {
    return (Array.isArray(raw) ? raw : [])
        .map(p => p.file_url || p.sample_url)
        .filter(u => u && _IMG_RE.test(u));
}

async function _poolSafebooru(queries) {
    for (const q of queries) {
        const tag = _normMultiTag(q); if (!tag) continue;
        const batch = new Set();
        for (const pid of [0, 1]) {
            try {
                const r = await axios.get(
                    `https://safebooru.org/index.php?page=dapi&s=post&q=index&json=1&limit=100&tags=${encodeBooruTags(tag)}&pid=${pid}`,
                    { timeout: 10000, headers: _HDR }
                );
                _parseSafebooruPosts(r.data).forEach(u => batch.add(u));
            } catch { }
        }
...
async function _poolYandere(queries) {
    for (const q of queries) {
        const tag = _normMultiTag(q); if (!tag) continue;
        const batch = new Set();
        for (const page of [1, 2]) {
            try {
                const r = await axios.get(
                    `https://yande.re/post.json?tags=${encodeBooruTags(tag)}&limit=100&page=${page}`,
                    { timeout: 10000, headers: _HDR }
                );
                _parseMoebooruPosts(r.data).forEach(u => batch.add(u));
            } catch { }
        }
        if (batch.size > 0) return [...batch];
    }
...
async function _poolKonachanCom(queries) {
    for (const q of queries) {
        const tag = _normMultiTag(q); if (!tag) continue;
        try {
            const r = await axios.get(
                `https://konachan.com/post.json?tags=${encodeBooruTags(tag)}&limit=100`,
                { timeout: 10000, headers: _HDR }
            );
            const batch = _parseMoebooruPosts(r.data);
...
async function _poolDanbooruSFW(queries) {
    for (const q of queries) {
        const tag = _normMultiTag(q); if (!tag) continue;
        try {
            const r = await axios.get(
                `https://danbooru.donmai.us/posts.json?tags=${encodeBooruTags(tag)}&limit=100`,
                { timeout: 12000, headers: _HDR }
            );
            const batch = (Array.isArray(r.data) ? r.data : [])
                .filter(p => p.file_url && _IMG_RE.test(p.file_url) && p.rating !== 'e')
                .map(p => p.file_url);
            if (batch.length > 0) return batch;
        } catch { }
    }
...
async function _poolPinterestGacha(nombre, serie) {
    try {
        const qs = [
            nombre && serie ? `${nombre} ${serie} anime` : null,
            nombre          ? `${nombre} anime`          : null,
        ].filter(Boolean);
        const all = new Set();
        for (const q of qs) {
            const pines = await buscarPinsDirecto(q);
            pines.filter(p => p.url).forEach(p => all.add(p.url));
        }
        return [...all];
    } catch { return []; }
}

async function _poolGelbooru(queries) {
    const GELBOORU_API_PARAMS = '&api_key=98f554258c88c44f4dd28ccde0c28f36682b2a992490ab35ebcc7baf7e196a86d7550b174bce577b8cc3f544e9b3ad0f6aeb09ad63bf89a9141cc3eddb6fbfd2&user_id=1917269';
    for (const q of queries) {
        const tag = _normMultiTag(q); if (!tag) continue;
        const batch = new Set();
        for (const pid of [0, 1]) {
            try {
                const r = await axios.get(
                    `https://gelbooru.com/index.php?page=dapi&s=post&q=index&json=1&limit=100&tags=${encodeBooruTags(tag)}&pid=${pid}${GELBOORU_API_PARAMS}`,
                    { timeout: 12000, headers: _HDR }
                );
                _parseGelbooruPosts(r.data).forEach(u => batch.add(u));
            } catch { }
        }
        if (batch.size > 0) return [...batch];
    }
...
async function _poolRule34(queries) {
    for (const q of queries) {
        const tag = _normMultiTag(q); if (!tag) continue;
        try {
            const r = await axios.get(
                `https://api.rule34.xxx/index.php?page=dapi&s=post&q=index&json=1&limit=100&tags=${encodeBooruTags(tag)}`,
                { timeout: 12000, headers: _HDR }
            );
            const batch = (Array.isArray(r.data) ? r.data : [])
                .filter(p => p.file_url && _IMG_RE.test(p.file_url))
                .map(p => p.file_url);
            if (batch.length > 0) return batch;
        } catch { }
    }
...
async function _poolDanbooruNSFW(queries) {
    for (const q of queries) {
        const tag = _normMultiTag(q); if (!tag) continue;
        try {
            const r = await axios.get(
...
async function _poolKonachanNet(queries) {
    for (const q of queries) {
        const tag = _normMultiTag(q); if (!tag) continue;
        try {
            const r = await axios.get(
                `https://konachan.net/post.json?tags=${encodeBooruTags(tag)}&limit=100`,
                { timeout: 10000, headers: _HDR }
            );
            const batch = _parseMoebooruPosts(r.data);
            if (batch.length > 0) return batch;
...
async function _poolTBIB(queries) {
    for (const q of queries) {
        const tag = _normMultiTag(q); if (!tag) continue;
        try {
            const r = await axios.get(
                `https://tbib.org/index.php?page=dapi&s=post&q=index&json=1&limit=100&tags=${encodeBooruTags(tag)}`,
                { timeout: 12000, headers: _HDR }
            );
            const batch = _parseGelbooruPosts(r.data);
            if (batch.length > 0) return batch;
        } catch { }
    }
    return [];
...
async function _poolXbooru(queries) {
    for (const q of queries) {
        const tag = _normMultiTag(q); if (!tag) continue;
        try {
            const r = await axios.get(
                `https://xbooru.com/index.php?page=dapi&s=post&q=index&json=1&limit=100&tags=${encodeBooruTags(tag)}`,
                { timeout: 12000, headers: _HDR }
            );
            const batch = _parseGelbooruPosts(r.data);
            if (batch.length > 0) return batch;
...
async function _poolHypnoHub(queries) {
    for (const q of queries) {
        const tag = _normMultiTag(q); if (!tag) continue;
        try {
            const r = await axios.get(
                `https://hypnohub.net/index.php?page=dapi&s=post&q=index&json=1&limit=100&tags=${encodeBooruTags(tag)}`,
                { timeout: 12000, headers: _HDR }
            );
            const batch = _parseGelbooruPosts(r.data);
            if (batch.length > 0) return batch;
...
async function _poolE621(queries) {
    for (const q of queries) {
        const tag = _normMultiTag(q); if (!tag) continue;
        try {
            const r = await axios.get(
                `https://e621.net/posts.json?tags=${encodeBooruTags(tag)}&limit=40`,
                { timeout: 12000, headers: { 'User-Agent': 'NexusBot/1.0 (gacha image search)' } }
            );
            const batch = (r.data?.posts || [])
                .filter(p => p.file?.url && _IMG_RE.test(p.file.url))
                .map(p => p.file.url);
            if (batch.length > 0) return batch;
        } catch { }
    }
...
// ── Paso 0: Pixiv — requiere PIXIV_REFRESH_TOKEN ────────────────────────────
//  Descarga el buffer directamente (pximg.net exige Referer de pixiv.net).
//  Devuelve un Buffer con la imagen, o null si no hay token / no hay resultados.
// ─────────────────────────────────────────────────────────────────────────────
async function _tryPixiv(queries, nsfw) {
    try {
        const cliente = await _pixivCliente();
        if (!cliente) return null;

        for (const q of queries.slice(0, 2)) {
            const searchQ = nsfw ? `${q} R-18` : q;
            try {
                // Página aleatoria (0-4) → cada llamada accede a una galería diferente.
                // La API devuelve 30 resultados por página; con offset 0,30,60,90,120
                // cubrimos hasta 150 ilustraciones distintas del mismo tag.
                // Si la página aleatoria viene vacía (tag nicho con pocos resultados),
                // se reintenta con offset 0 antes de descartar el query.
                const offsetAleatorio = Math.floor(Math.random() * 5) * 30;
                const offsetsAIntentar = offsetAleatorio === 0
                    ? [0]
                    : [offsetAleatorio, 0];   // fallback a primera página si la aleatoria está vacía

                let ilusts = [];
                let offsetUsado = 0;
                for (const offset of offsetsAIntentar) {
                    const res = await cliente.searchIllust(searchQ, {
                        search_target: 'partial_match_for_tags',
                        sort: 'popular_desc',
                        offset,
                    });
                    ilusts = (res?.illusts || []).filter(i => i.type === 'illust' || i.type === 'manga');
                    if (ilusts.length) { offsetUsado = offset; break; }
                }
                if (!ilusts.length) continue;

                // Elegir al azar de TODOS los resultados de la página (no solo los primeros 15)
                const il = ilusts[Math.floor(Math.random() * ilusts.length)];
                const url = il.image_urls?.large || il.image_urls?.medium;
                if (!url) continue;

                const buf = await _pixivDescargar(url);
                if (buf?.length > 2000) {
                    console.log(`[Pixiv] ✅ "${q}"${nsfw ? ' [NSFW]' : ''} → ${il.title} (❤️${il.total_bookmarks}) [offset:${offsetUsado}]`);
                    return buf;
                }
            } catch (e) {
                console.warn(`[Pixiv] ⚠️ "${q}" → ${e.message}`);
            }
        }
        return null;
    } catch { return null; }
...
// ── Búsqueda de imagen con pools paralelos ────────────────────────────────────
//
//  PASO 0: Pixiv (si hay PIXIV_REFRESH_TOKEN) → devuelve { buffer }
//  PASO 1: Todas las demás fuentes en PARALELO → devuelve { url }
//  Gana la fuente con MÁS imágenes; se devuelve una URL al azar de ese pool.
//  Modo SFW:  Safebooru + Yande.re + Konachan.com + Danbooru(SFW) + Pinterest
//  Modo NSFW: Gelbooru + Rule34 + Danbooru(all) + Konachan.net +
//             TBIB + Xbooru + HypnoHub + e621 + (SFW como respaldo)
//
//  RETORNO: { buffer: Buffer } | { url: string } | null
// ──────────────────────────────────────────────────────────────────────────────
async function buscarImagenPersonajeConFallback(personaje, nsfw = false) {
    const tag     = (personaje.tag    || '').trim();
    const nombre  = (personaje.nombre || '').trim();
    const serie   = (personaje.serie  || '').trim();
    const aliases = personaje.alias || personaje.aliases || [];

    // ── Construir variantes de búsqueda ────────────────────────────────────
    //  Orden: de más específico a más genérico.
    //  1. nombre+serie como AND-tag (más preciso, funciona en todos los boorus)
    //  2. tag exacto del DB (ej: ram_(re_zero)) — funciona en boorus que usan guión bajo
    //  3. nombre solo
    //  4. aliases
    //  5. tag sin sufijo de serie
    //  6. primer nombre (solo SFW — más genérico, último recurso)
    const vistos = new Set();
    const queries = [];
    const addQ = v => {
        const t = _normTag(v);
        if (t.length > 1 && !vistos.has(t)) { vistos.add(t); queries.push(t); }
    };
    // 1° — nombre+serie como AND-tag (mayor precisión)
    if (nombre && serie) {
        const mt = `${_normTag(nombre)} ${serieATagBooru(serie)}`;
        if (mt.length > 1 && !vistos.has(mt)) { vistos.add(mt); queries.push(mt); }
    }
    addQ(tag);                                      // 2° — tag exacto del DB
    addQ(nombre);                                   // 3° — nombre solo
    for (const a of (Array.isArray(aliases) ? aliases : [])) addQ(String(a));  // 4°
    addQ(tag.replace(/_?\([^)]*\)$/, '').trim());   // 5° — tag sin sufijo de serie
    addQ(nombre.split(/[\s_]+/)[0]);                // 6° — primer nombre (último recurso)

    // ── PASO 0: Pixiv — solo en modo NSFW ─────────────────────────────────
    //  En modo SFW se omite Pixiv completamente y se va directo a los scrapers.
    //  En modo NSFW se reordena el array de queries poniendo "nombre + serie"
    //  primero para evitar confusiones entre personajes con el mismo nombre.
    if (nsfw) {
        const pixivQueries = [];
        const pqVisto = new Set();
        // _normMultiTag preserva el espacio → Pixiv lo interpreta como AND entre tags:
        //   "yor_forger spy_x_family" → ilustraciones con ambos tags al mismo tiempo.
        // _normTag colapsaba a "yor_forger_spy_x_family" (un solo tag malformado
        // que hace partial_match con contenido irrelevante).
        const addPQ = v => {
            const t = _normMultiTag(v);
            if (t.length > 1 && !pqVisto.has(t)) { pqVisto.add(t); pixivQueries.push(t); }
        };
        // 1° — nombre + serie como dos tags AND (máxima precisión en Pixiv)
        if (nombre && serie) addPQ(`${_normTag(nombre)} ${serieATagBooru(serie)}`);
        // 2° — nombre solo (siempre funciona aunque la serie no esté tagueada)
        addPQ(_normTag(nombre) || _normTag(tag));
        // 3° — resto de queries ya construidas
        for (const q of queries) addPQ(q);

        const pixivBuf = await _tryPixiv(pixivQueries, true);
        if (pixivBuf) return { buffer: pixivBuf };
    }

    // ── Seleccionar fuentes según modo ─────────────────────────────────────
    const sfwSources  = [
        { name: 'safebooru',    fn: () => _poolSafebooru(queries)       },
        { name: 'konachan.com', fn: () => _poolKonachanCom(queries)     },
        { name: 'danbooru-sfw', fn: () => _poolDanbooruSFW(queries)     },
        { name: 'pinterest',    fn: () => _poolPinterestGacha(nombre, serie) },
    ];
    // ── Queries NSFW: específicas, sin primer nombre suelto ───────────────────
    //  El nombre suelto (ej: "yor") es demasiado genérico en fuentes NSFW y
    //  provoca que el pool gane con contenido de personajes incorrectos.
    //  Se mantiene nombre+serie (AND-tag) como query más precisa al frente.
    const queriesNsfw = [];
    const nsfwVisto   = new Set();
    const addQN = t => { if (t.length > 1 && !nsfwVisto.has(t)) { nsfwVisto.add(t); queriesNsfw.push(t); } };
    if (nombre && serie) addQN(`${_normTag(nombre)} ${serieATagBooru(serie)}`); // AND-tag preciso
    addQN(_normTag(tag));
    addQN(_normTag(nombre));
    for (const a of (Array.isArray(aliases) ? aliases : [])) addQN(_normTag(String(a)));
    addQN(_normTag(tag.replace(/_?\([^)]*\)$/, '').trim()));
    // ⚠️ NO se incluye nombre suelto (primer nombre) — demasiado genérico en NSFW

    const nsfwSources = [
        { name: 'gelbooru',     fn: () => _poolGelbooru(queriesNsfw)        },
        { name: 'rule34',       fn: () => _poolRule34(queriesNsfw)          },
        { name: 'danbooru',     fn: () => _poolDanbooruNSFW(queriesNsfw)    },
        { name: 'yande.re',     fn: () => _poolYandere(queriesNsfw)         },
        { name: 'konachan.net', fn: () => _poolKonachanNet(queriesNsfw)     },
        { name: 'tbib',         fn: () => _poolTBIB(queriesNsfw)            },
        { name: 'xbooru',       fn: () => _poolXbooru(queriesNsfw)          },
        { name: 'hypnohub',     fn: () => _poolHypnoHub(queriesNsfw)        },
        { name: 'e621',         fn: () => _poolE621(queriesNsfw)            },
    ];

    const fuentes = nsfw ? [...nsfwSources, ...sfwSources] : sfwSources;

    // ── Ejecutar todas las fuentes en PARALELO ─────────────────────────────
    const resultados = await Promise.allSettled(fuentes.map(s => s.fn()));

    const pools = resultados
        .map((r, i) => ({
            nombre: fuentes[i].name,
            urls: (r.status === 'fulfilled' && Array.isArray(r.value)) ? r.value : [],
        }))
        .filter(p => p.urls.length > 0);

    if (!pools.length) {
        console.log(`[Personaje]${nsfw ? '[NSFW]' : ''} ❌ "${nombre}" (${serie}) → sin imagen (${fuentes.length} fuentes sin resultados)`);
        return null;
    }

    // ── Elegir la fuente de mayor prioridad que devolvió resultados ────────
    //  pools preserva el orden de fuentes[] (Promise.allSettled + filter no reordena),
    //  por lo que pools[0] es siempre la fuente con mayor prioridad que tuvo resultados.
    //  Antes se usaba "más imágenes gana", lo que permitía que una fuente grande con
    //  resultados genéricos ganara sobre una fuente pequeña con resultados precisos.
    const mejor = pools[0];
    const url   = mejor.urls[Math.floor(Math.random() * mejor.urls.length)];

    const resumen = pools.map(p => `${p.nombre}:${p.urls.length}`).join(' | ');
    console.log(`[Personaje]${nsfw ? '[NSFW]' : ''} ✅ "${nombre}" (${serie}) → ${mejor.nombre} (${mejor.urls.length} imgs, 1° de ${pools.length} fuentes) [${resumen}]`);
...
function extraerNombreCitado(msg) {
    ...
    // El formato incluye "*Nombre »*" y la instrucción "#claim"
    const esRoll = texto.includes('Nombre »') && (texto.includes('#claim') || texto.includes('#c'));
...
function formatCaption(p, groupId) {
    ...
    // Calcular puesto global del personaje por valor descendente (cacheado)
    const lista = getPersonajes();
    const sorted = [...lista].sort((a, b) => (b.valor || 0) - (a.valor || 0));
    const puesto = sorted.findIndex(x => x.nombre === p.nombre) + 1;
    const puestoStr = puesto > 0 ? `#${puesto}` : '—';

    // Último voto global (campo persistido en personajes.json)
    const ultimoVotoStr = _tiempoRelativo(p.ultimoVotoTs || 0);

    return [
        `❋ *Nombre »* ${p.nombre}`,
        `${generoIcon} *Género »* ${p.genero || '—'}`,
        `☆ *Valor »* ${(p.valor || 0).toLocaleString()}`,
        `⛁ *Precio »* ¥${((p.valor || 0) * CLAIM_COST_MULTIPLIER).toLocaleString()} Nexcoins`,
        `♡ *Estado »* ${estado}`,
        `◆ *Fuente »* ${p.serie || '—'}`,
        `❖ *Puesto »* ${puestoStr}`,
        `✗ *Último voto »* ${ultimoVotoStr}`,
        ``,
        `> _Cita y usa_ *#claim* _o_ *#c* _para reclamarlo_`,
    ].join('\n');
}
...
async function manejarMensajePersonajes(sock, msg) {
    ...
    switch (cmd) {
        ...
        case 'rollwaifu': {
            ...
            if (ahora - ultimo < cooldownTiempo) {
                ...
                return sock.sendMessage(jid, {
                    text: `${WARN} Próximo roll en *${msToTime(restante)}*\n${barraCooldown(restante, cooldownTiempo)}`
                }, Q);
            }

            if (!lista.length) return sock.sendMessage(jid, { text: `${ERR} No hay personajes disponibles por ahora.` }, Q);

            const personaje = rollPonderado(lista);

            // Guardar cooldown (lastRoll ya no se usa para claim — la citación es obligatoria)
            ...
            // Helper para fijar el roll activo en el grupo (fuente de verdad para #claim)
            const fijarRollActivo = () => {
                // Registrar en el Map en memoria — el claim siempre valida contra esto
            ...
            const imgResult = await buscarImagenPersonajeConFallback(personaje, modoNSFW);

            fijarRollActivo();

            if (imgResult?.buffer) {
                return sock.sendMessage(jid, { image: imgResult.buffer, caption }, Q);
            } else if (imgResult?.url) {
                return sock.sendMessage(jid, { image: { url: imgResult.url }, caption }, Q);
            }
        ...
        case 'reclamar': {
            ...
            const { u: uFresco } = getUsuarioPersonajes(senderJid);
            ...
            if (ahora - ultimoClaim < CLAIM_COOLDOWN) {
                ...
                return sock.sendMessage(jid, {
                    text: `${WARN} Ya tienes una carta, espera *${msToTime(restante)}* antes de reclamar otra.\n${barraCooldown(restante, CLAIM_COOLDOWN)}`
                }, Q);
            }

            // ── SIEMPRE requiere citar la carta del roll ─────────────────────
            // Esto evita el bug donde el lastRoll guardado de un roll anterior
            // interfiere al intentar reclamar el personaje de otro usuario.
            const nombreCitado = extraerNombreCitado(msg);
            if (!nombreCitado) {
                return sock.sendMessage(jid, {
                    text: `${INFO} Cita la carta del roll y luego escribe *#c*.\n\n_Usa *#rw* para rollear, cita el resultado y usa *#c*._`
                }, Q);
            }

            // ── Validar roll activo en el grupo ──────────────────────────────
            const pendingRoll = pendingGroupRolls.get(groupId);
            if (!pendingRoll) {
                return sock.sendMessage(jid, {
                    text: `${ERR} No hay ningún roll activo ahora mismo.\nUsa *#rw* para rollear un personaje.`
                }, Q);
            }
            if (pendingRoll.nombre.toLowerCase() !== nombreCitado.toLowerCase()) {
                return sock.sendMessage(jid, {
                    text: `${ERR} El roll activo es *${pendingRoll.nombre}*, no *${nombreCitado}*.\nCita la carta correcta o espera el próximo *#rw*.`
                }, Q);
            }
            if (ahora - pendingRoll.rollTime > ROLL_EXPIRACION) {
                ...
                _persistirRolls();
                return sock.sendMessage(jid, {
                    text: `${WARN} *${nombreCitado}* ya no está disponible, se fue. Usa *#rw* para rollear otro.`
                }, Q);
            }
            ...
            if (senderJid !== pendingRoll.rollerJid && ahora - pendingRoll.rollTime < CLAIM_PRIORITY_WINDOW) {
                ...
                return sock.sendMessage(jid, {
                    text: `${WARN} Espera *${esperaS}s*, quien rolleó tiene prioridad.`
                }, Q);
            }

            // ── Buscar personaje en la lista (coincidencia exacta) ───────────
            const nombre = nombreCitado;
            const personaje = lista.find(p =>
                p.nombre.toLowerCase() === nombre.toLowerCase()
            );
            if (!personaje) {
                return sock.sendMessage(jid, { text: `${ERR} Personaje *${nombre}* no encontrado en la base de datos.` }, Q);
            }
            ...
            if (dueno && dueno !== senderJid) {
                return sock.sendMessage(jid, {
                    text: `${WARN} *${personaje.nombre}* ya fue reclamado en este grupo por \`${dueno.split('@')[0]}\`.\nUsa *#rw* para intentar con otro.`
                }, Q);
            }
            if (estaEnHarem(getHaremGrupo(uFresco, groupId), personaje) || estaEnHarem(uFresco.harem || [], personaje)) {
                return sock.sendMessage(jid, { text: `${WARN} Ya tienes a *${personaje.nombre}* en tu harem.` }, Q);
            }

            // ── Verificar coins ──────────────────────────────────────────────
            const costoReclamar = personaje.valor * CLAIM_COST_MULTIPLIER;
            if (uFresco.monedas < costoReclamar) {
                return sock.sendMessage(jid, {
                    text: `${ERR} Necesitas *${costoReclamar.toLocaleString()} coins* y tienes *${uFresco.monedas.toLocaleString()}*.\n_Gana coins con *#diario*, *#work* o *#crime*._`
                }, Q);
            }

            const nuevoBalance = uFresco.monedas - costoReclamar;
            ...
            guardarUsuarioSeguro(senderJid, (uGuardado) => {
                ...
                // Limpiar lastRoll por compatibilidad (ya no se usa para claim)
                uGuardado.lastRoll = null;
                uGuardado.lastRollTime = null;
                uGuardado.lastRollGroupId = null;
            });

            // Limpiar roll activo del grupo
            pendingGroupRolls.delete(groupId);
            _persistirRolls();

            // Prioridad: mensaje del grupo → mensaje global → predeterminado
            const claimTemplate = (uFresco.claimMsgGrupo && uFresco.claimMsgGrupo[groupId])
                || uFresco.claimMsg
                || null;
            const claimMsg = claimTemplate
                ? claimTemplate
                    .replace(/\{nombre\}/g, personaje.nombre)
                    .replace(/\{usuario\}/g, pushName || senderJid.split('@')[0])
                : `${OK} *${personaje.nombre}* es tuya/tuyo!\n${FE} -${costoReclamar.toLocaleString()} coins · Saldo: *${nuevoBalance.toLocaleString()}*`;
        ...
        case 'claims': {
            ...
            if (!harem || harem.length === 0) {
                return sock.sendMessage(jid, { text: objetivoJid === senderJid ? `${INFO} No tienes personajes... usa *#rw* para obtener uno.` : `${INFO} \`${nombreMostrar}\` no tiene personajes.` }, Q);
            }
            ...
            let textoHarem = `${H(`Harem de ${nombreMostrar}`)}\n\n`;
        ...
        case 'delchar': {
            ...
            if (!nombre) return sock.sendMessage(jid, { text: `${ERR} Uso: #delchar [nombre]` }, Q);
            ...
            if (idxDel === -1) return sock.sendMessage(jid, { text: `${ERR} No tienes a *${nombre}* en tu harem.` }, Q);
            if (delFromLegacy) u.harem.splice(idxDel, 1);
            else haremDel.splice(idxDel, 1);
            usuarios[senderJid] = u;
            guardarUsuarios(usuarios);
            await sock.sendMessage(jid, { text: `${OK} *${nombre}* fue eliminado de tu harem.` }, Q);
            break;
        }
        ...
        case 'regalar': {
            ...
            if (!mencionados.length || !args.length) {
                return sock.sendMessage(jid, { text: `${ERR} Uso: #givechar @usuario [nombre]` }, Q);
            }
            ...
            if (idxGive === -1) return sock.sendMessage(jid, { text: `${ERR} No tienes a *${nombre}* en tu harem.` }, Q);
            ...
            await sock.sendMessage(jid, {
                text: `${OK} \`${pushName}\` regaló a *${nombre}* a \`${objetivoJid.split('@')[0]}\``
            }, Q);
            break;
        }
        ...
        case 'giveallharem': {
            const mencionados = (msg.message.extendedTextMessage?.contextInfo?.mentionedJid || [])
                .map(j => resolverJid(j.replace(/:\d+@/, '@')))
                .filter(j => j && !j.endsWith('@g.us') && !j.endsWith('@broadcast'));
            if (!mencionados.length) return sock.sendMessage(jid, { text: `${ERR} Uso: #giveallharem @usuario` }, Q);
            const haremGiveAll = getHaremGrupo(u, groupId);
            const legacyGiveAll = (u.harem || []);
            if (!haremGiveAll.length && !legacyGiveAll.length) return sock.sendMessage(jid, { text: `${ERR} Tu harem está vacío.` }, Q);
            ...
            guardarUsuarios(usuarios);
            await sock.sendMessage(jid, {
                text: `${OK} \`${pushName}\` regaló todos sus personajes (${total}) a \`${objetivoJid.split('@')[0]}\``
            }, Q);
            break;
        }
        ...
        case 'vender': {
            ...
            if (isNaN(precio) || precio <= 0 || !nombre) {
                return sock.sendMessage(jid, { text: `${ERR} Uso: #sell [precio] [nombre]\nEjemplo: #sell 500 Rem` }, Q);
            }
            ...
            if (idxSell === -1) return sock.sendMessage(jid, { text: `${ERR} No tienes a *${nombre}* en tu harem.` }, Q);
            ...
            await sock.sendMessage(jid, { text: `${OK} *${pNombre}* fue puesto a la venta por *${precio} coins*!\n\n_Usa *#haremshop* para ver la tienda y *#buyshop [n]* para comprarlo._` }, Q);
            break;
        }
        ...
        case 'removerventa': {
            const nombre = args.join(' ');
            if (!nombre) return sock.sendMessage(jid, { text: `${ERR} Uso: #removesale [nombre]` }, Q);
            const ventasRemove = getVentasGrupo(u, groupId);
            if (!ventasRemove.length) return sock.sendMessage(jid, { text: `${ERR} No tienes personajes en venta en este grupo.` }, Q);
            ...
            if (idx === -1) return sock.sendMessage(jid, { text: `${ERR} No tienes *${nombre}* en venta.` }, Q);
            ...
            usuarios[senderJid] = u;
            guardarUsuarios(usuarios);
            await sock.sendMessage(jid, { text: `${OK} *${nombre}* fue retirado de la venta y devuelto a tu harem.` }, Q);
            break;
        }
        ...
        case 'wshop': {
            ...
            if (!ventas.length) return sock.sendMessage(jid, { text: `${INFO} La tienda de personajes de este grupo está vacía.\n\n_Usa *#sell [precio] [nombre]* para vender tus personajes._` }, Q);
            ...
            ventasPagina.forEach((v, i) => {
                ...
                texto += `${inicio + i + 1}. *${nombre}* _(${serie})_\n   ${FE} *${v.precio} coins* — \`${vendedorNombre}\`\n\n`;
            });
            texto += `${FI} Página *${paginaReal}/${totalPaginas}* · Total: *${ventas.length}* personajes\n`;
            if (totalPaginas > 1) texto += `_Usa *#haremshop [página]* para navegar_\n`;
            texto += `\n_Usa *#buyshop [número]* para comprar_`;
        ...
        case 'intercambiar': {
            ...
            if (!mencionadosTrade.length || partes.length < 2 || !partes[0] || !partes[1]) {
                return sock.sendMessage(jid, { text: `${ERR} Uso: #trade @usuario [tu personaje] / [personaje que quieres]\nEjemplo: #trade @amigo Rem / Nezuko\n\n_El usuario objetivo debe aceptar con *#accepttrade*._` }, Q);
            }
            ...
            if (objetivoJidProp === senderJid) {
                return sock.sendMessage(jid, { text: `${ERR} No puedes hacer un intercambio contigo mismo.` }, Q);
            }
            ...
            if (miIdxProp === -1) {
                ...
                if (legIdx === -1) return sock.sendMessage(jid, { text: `${ERR} No tienes a *${miNombreProp}* en tu harem.` }, Q);
            } else {
            ...
            if (misVentas.some(v => (typeof v === 'string' ? v : v.nombre)?.toLowerCase() === miNombreReal.toLowerCase())) {
                return sock.sendMessage(jid, { text: `${ERR} *${miNombreReal}* está en venta. Retíralo del mercado antes de intercambiarlo.` }, Q);
            }
            ...
            if (suIdxProp === -1) {
                ...
                if (legIdx2 === -1) return sock.sendMessage(jid, { text: `${ERR} \`${objetivoJidProp.split('@')[0]}\` no tiene a *${suNombreProp}* en su harem.` }, Q);
            } else {
            ...
            if (suVentas.some(v => (typeof v === 'string' ? v : v.nombre)?.toLowerCase() === suNombreReal.toLowerCase())) {
                return sock.sendMessage(jid, { text: `${ERR} *${suNombreReal}* está en venta por \`${objetivoJidProp.split('@')[0]}\`. Cómpralo con #buychar.` }, Q);
            }
            ...
            await sock.sendMessage(jid, {
                text: `${H('Propuesta de intercambio')}\n\n` +
                      `${FC} \`${pushName}\` ofrece: *${miNombreReal}*\n` +
                      `${FA} Solicita a \`${objetivoJidProp.split('@')[0]}\`: *${suNombreReal}*\n\n` +
                      `_\`${objetivoJidProp.split('@')[0]}\`, usa *#accepttrade* para aceptar o *#canceltrade* para rechazar._\n` +
        ...
        case 'confirmtrade': {
            ...
            if (!trade) {
                return sock.sendMessage(jid, { text: `${ERR} No tienes ninguna propuesta de intercambio pendiente.\n_El proponente debe enviarte un #trade primero._` }, Q);
            }
            if (Date.now() - trade.timestamp > TRADE_EXPIRA) {
                pendingTrades.delete(tradeKeyAcept);
                return sock.sendMessage(jid, { text: `${WARN} La propuesta de intercambio expiró (3 minutos). Pídele al otro usuario que la reenvíe.` }, Q);
            }
            ...
            if (propIdx === -1) {
                pendingTrades.delete(tradeKeyAcept);
                return sock.sendMessage(jid, { text: `${ERR} El proponente ya no tiene a *${propNombre}*. Intercambio cancelado.` }, Q);
            }
            ...
            if (acepIdx === -1) {
                pendingTrades.delete(tradeKeyAcept);
                return sock.sendMessage(jid, { text: `${ERR} Ya no tienes a *${acepNombre}* en tu harem. Intercambio cancelado.` }, Q);
            }

            // Verificar duplicados
            if (estaEnHarem(propHaremG, acepNombre) || estaEnHarem(uProposer.harem || [], acepNombre)) {
                pendingTrades.delete(tradeKeyAcept);
                return sock.sendMessage(jid, { text: `${WARN} El proponente ya tiene a *${acepNombre}*. Intercambio cancelado.` }, Q);
            }
            if (estaEnHarem(acepHaremG, propNombre) || estaEnHarem(uAceptor.harem || [], propNombre)) {
                pendingTrades.delete(tradeKeyAcept);
                return sock.sendMessage(jid, { text: `${WARN} Ya tienes a *${propNombre}*. Intercambio cancelado.` }, Q);
            }
            ...
            await sock.sendMessage(jid, {
                text: `${OK} *Intercambio completado*\n\n` +
                      `${FC} \`${proposerJid.split('@')[0]}\` dio *${propNombre}* y recibió *${acepNombre}*\n` +
                      `${FC} \`${senderJid.split('@')[0]}\` dio *${acepNombre}* y recibió *${propNombre}*`
            }, Q);
            break;
        }
        ...
        case 'rechazartrade': {
            ...
            if (canceledKey) {
                ...
                return sock.sendMessage(jid, { text: `${ERR} Tu propuesta de intercambio fue cancelada.` }, Q);
            }
            return sock.sendMessage(jid, { text: `${INFO} No tienes ninguna propuesta de intercambio activa.` }, Q);
        }
        ...
        case 'infogacha': {
            ...
            await sock.sendMessage(jid, {
                text: `${H('Gacha Info')}\n\n` +
                      `${F} Personajes en el bot: *${lista.length}*\n` +
                      `${FE} Tus monedas: *${u.monedas}*\n` +
                      `${FI} Tu harem: ${totalStr} personajes\n` +
                      `${F} En tienda (grupo): *${enVenta}*\n` +
                      `${WARN} Roll cooldown: ${ahora - (u.cooldowns.rw || 0) < cooldownTiempo ? msToTime(cooldownTiempo - (ahora - (u.cooldowns.rw || 0))) : '¡Listo!'}`
            }, Q);
            break;
        }
        ...
        case 'wimage': {
            const nombre = args.join(' ');
            if (!nombre) return sock.sendMessage(jid, { text: `${ERR} Uso: #charimage [nombre]` }, Q);
            ...
            if (!personaje) return sock.sendMessage(jid, { text: `${ERR} Personaje *${nombre}* no encontrado` }, Q);

            const imgPath = path.join(__dirname, 'img', personaje.imagen || '');
            if (personaje.imagen && fs.existsSync(imgPath)) {
                return sock.sendMessage(jid, { image: fs.readFileSync(imgPath), caption: `◈ *${personaje.nombre}* — ${personaje.serie}` }, Q);
            }

            await sock.sendMessage(jid, { text: `${INFO} Buscando imagen de *${personaje.nombre}*...` }, Q);
            const imgResult2 = await buscarImagenPersonajeConFallback(personaje, modoNSFW);
            if (imgResult2?.buffer) return sock.sendMessage(jid, { image: imgResult2.buffer, caption: `◈ *${personaje.nombre}* — ${personaje.serie}` }, Q);
            if (imgResult2?.url)    return sock.sendMessage(jid, { image: { url: imgResult2.url }, caption: `◈ *${personaje.nombre}* — ${personaje.serie}` }, Q);
            await sock.sendMessage(jid, { text: `${ERR} No encontré imagen para *${personaje.nombre}*` }, Q);
            break;
        }
        ...
        case 'waifuinfo': {
            const nombre = args.join(' ');
            if (!nombre) return sock.sendMessage(jid, { text: `${ERR} Uso: #charinfo [nombre]` }, Q);
            ...
            const precioInfo = (personaje.valor * CLAIM_COST_MULTIPLIER).toLocaleString();
            // Mismos datos que muestra #rw (valor, puesto global y último voto) para que
            // #winfo sea una versión completa de la ficha del personaje.
            const sortedInfo = [...lista].sort((a, b) => (b.valor || 0) - (a.valor || 0));
            const puestoInfo = sortedInfo.findIndex(x => x.nombre === personaje.nombre) + 1;
            const puestoInfoStr = puestoInfo > 0 ? `#${puestoInfo}` : '—';
            const ultimoVotoInfoStr = _tiempoRelativo(personaje.ultimoVotoTs || 0);
            const captionInfo = `${F} *Nombre »* ${personaje.nombre}\n⚥ *Género »* ${personaje.genero}\n☆ *Valor »* ${(personaje.valor || 0).toLocaleString()}\n⛁ *Precio »* ¥${precioInfo} Nexcoins\n${OK} *Estado »* ${duenoJid ? 'Reclamado' : 'Libre'}\n${F} *Fuente »* ${personaje.serie}${duenoTxt ? `\n${FC} *Dueño »* ${duenoTxt}` : ''}\n❖ *Puesto »* ${puestoInfoStr}\n✗ *Último voto »* ${ultimoVotoInfoStr}`;
            const mentions = [];
            ...
            const imgResult3 = await buscarImagenPersonajeConFallback(personaje, modoNSFW);
            if (imgResult3?.buffer) {
                return sock.sendMessage(jid, { image: imgResult3.buffer, caption: captionInfo, mentions }, Q);
            } else if (imgResult3?.url) {
                return sock.sendMessage(jid, { image: { url: imgResult3.url }, caption: captionInfo, mentions }, Q);
            }
        ...
        case 'wtop': {
            ...
            let texto = `${H('Top Personajes')}\n\n`;
            top.forEach((p, i) => {
                const genIco = p.genero?.toLowerCase() === 'mujer' ? '♀' : p.genero?.toLowerCase() === 'hombre' ? '♂' : '⚧';
                const { nombre: rNombre } = getRareza(p.valor);
                texto += `${i + 1}. ◈ *${p.nombre}* ⚥ _(${p.serie})_ — ✦ ${p.valor} | ${(p.valor * CLAIM_COST_MULTIPLIER).toLocaleString()} coins\n`;
            });
            await sock.sendMessage(jid, { text: texto }, Q);
            break;
        }
        ...
        case 'buyc': {
            ...
            if (!ventasShop.length) {
                return sock.sendMessage(jid, { text: `${INFO} La tienda de personajes de este grupo está vacía actualmente.` }, Q);
            }
            if (num > ventasShop.length) {
                return sock.sendMessage(jid, {
                    text: `${ERR} Solo hay *${ventasShop.length}* personaje(s) en la tienda. Usa *#haremshop* para ver la lista.`
                }, Q);
            }
            ...
            if (vendedorId === senderJid) {
                return sock.sendMessage(jid, { text: `${ERR} No puedes comprarte tu propio personaje.` }, Q);
            }
            ...
            if (estaEnHarem(getHaremGrupo(u, groupId), personajeCompraObj) || estaEnHarem(u.harem || [], personajeCompraObj)) {
                return sock.sendMessage(jid, { text: `${WARN} Ya tienes a *${nombreCompra}* en tu harem.` }, Q);
            }
            if (u.monedas < entrada.precio) {
                return sock.sendMessage(jid, {
                    text: `${ERR} Necesitas *${entrada.precio.toLocaleString()} coins* para comprar a *${nombreCompra}*.\n${FE} Tienes: *${u.monedas.toLocaleString()} coins*\n_Gana coins con *#diario*, *#work* o *#crime*._`
                }, Q);
            }
            ...
            if (idxVenta === -1) {
                return sock.sendMessage(jid, { text: `${WARN} *${nombreCompra}* ya fue comprado por alguien más.` }, Q);
            }
            ...
            return sock.sendMessage(jid, {
                text: `${OK} *Compra exitosa*\n\n${F} *${nombreCompra}*\n${FS} ${personajeDataB?.serie || '?'}\n\n${FE} Pagaste: *${entrada.precio.toLocaleString()} coins* a \`${vendedorNombre}\`\n${FE} Te quedan: *${comprador.monedas.toLocaleString()} coins*`
            }, Q);
        }
        ...
        case 'favtop': {
            ...
            if (nombreFav) {
                ...
                if (!personaje) return sock.sendMessage(jid, { text: `${ERR} Personaje *${nombreFav}* no encontrado.` }, Q);
                u.favorito = personaje.nombre;
                usuarios[senderJid] = u;
                guardarUsuarios(usuarios);
                return sock.sendMessage(jid, { text: `${OK} Tu personaje favorito ahora es *${personaje.nombre}*.` }, Q);
            }
            const todos = cargarUsuarios();
            const conteo = {};
            ...
            if (!top.length) return sock.sendMessage(jid, { text: `${ERR} Nadie ha establecido un favorito aún.` }, Q);
            let texto = `${H('Top Favoritos')}\n\n`;
            top.forEach(([nombre, votos], i) => {
                texto += `${i + 1}. *${nombre}* — ✦ ${votos} votos\n`;
            });
            await sock.sendMessage(jid, { text: texto }, Q);
            break;
        }
        ...
        case 'animeinfo': {
            ...
            const nombre = args.join(' ');
            if (!nombre) return sock.sendMessage(jid, { text: `${ERR} Uso: #ainfo [nombre de la serie]` }, Q);
            const personajesSerie = lista.filter(p => p.serie.toLowerCase().includes(nombre.toLowerCase()));
            if (!personajesSerie.length) return sock.sendMessage(jid, { text: `${ERR} No se encontraron personajes de *${nombre}*` }, Q);
            const serie = personajesSerie[0].serie;
            const POR_PAG = 25;
            const { items: pagPersonajes, pag, totalPags } = paginar(personajesSerie, _paginaP, POR_PAG);

            // Contar reclamados en este grupo
            const totalPersonajes = personajesSerie.length;
            const reclamados = personajesSerie.filter(p => obtenerDuenoPersonajeGrupo(p, groupId)).length;
            const pctRec = totalPersonajes > 0 ? Math.round((reclamados / totalPersonajes) * 100) : 0;

            let texto = `${H('Serie Info')}\n\n`;
            texto += `✦ *Nombre:* <${serie}>\n\n`;
            texto += `□ *Personajes »* ${totalPersonajes}\n`;
            texto += `♡ *Reclamados »* ${reclamados}/${totalPersonajes} (${pctRec}%)\n`;
            texto += `□ *Lista de personajes:*\n`;

            let listaTexto = '\n';
            pagPersonajes.forEach(p => {
                const estado = obtenerDuenoPersonajeGrupo(p, groupId) ? 'Reclamado' : 'Libre';
                listaTexto += `» ${p.nombre} (${p.valor}) • ${estado}.\n`;
            });
            texto += listaTexto;
        ...
        case 'animelist': {
            ...
            const { items: seriesPag, pag, totalPags } = paginar(series, _paginaP, 20);
            let texto = `${H('Lista de Series')}\n\n`;
            texto += `□ *Lista de series* (${series.length}):\n\n`;
            seriesPag.forEach(s => {
            ...
            if (totalPags > 1) {
                texto += `\n> • Página ${pag}/${totalPags}`;
                if (pag > 1)         texto += `  ← #slist${pag - 1}`;
                if (pag < totalPags) texto += `  #slist${pag + 1} →`;
            }
            await sock.sendMessage(jid, { text: texto }, Q);
            break;
        }
        ...
        case 'colec': {
            ...
            if (!nombreSerie) {
                return sock.sendMessage(jid, { text: `${ERR} Uso: *#coleccion [nombre de la serie]*\nEjemplo: \`#coleccion Sword Art Online\`` }, Q);
            }
            const personajesSerie = lista.filter(p =>
                p.serie.toLowerCase().includes(nombreSerie.toLowerCase())
            );
            if (!personajesSerie.length) {
                return sock.sendMessage(jid, { text: `${ERR} No se encontraron personajes de *${nombreSerie}*.\n\n_Usa *#serielist* para ver todas las series disponibles._` }, Q);
            }
            ...
            let texto = `${H('Colección')}\n\n`;
            texto += `${FS} *${serieCanonica}*\n`;
            texto += `${FC} \`${senderJid.split('@')[0]}\`\n`;
            texto += `${FI} Progreso: *${tieneCount}/${total}* (${porcentaje}%)\n`;
            const barLen = 15;
        ...
        case 'votar': {
            const nombre = args.join(' ');
            if (!nombre) return sock.sendMessage(jid, { text: `${ERR} Uso: #vote [nombre]` }, Q);
            ...
            if (ahora - ultimoVoto < 24 * 60 * 60 * 1000) {
                return sock.sendMessage(jid, { text: `${WARN} Ya votaste por *${personaje.nombre}* hoy. Vuelve mañana.` }, Q);
            }
            personaje.valor = (personaje.valor || 0) + 10;
            personaje.ultimoVotoTs = ahora;
            u.votosPersonaje[personaje.nombre] = ahora;
            usuarios[senderJid] = u;
            guardarUsuarios(usuarios);
            guardarJSON(personajesPath, data);
            invalidarCache();
            await sock.sendMessage(jid, { text: `${OK} Votaste por *${personaje.nombre}*. Su valor aumentó a *${personaje.valor.toLocaleString()}* ☆` }, Q);
            break;
        }

        // 💬 SETEAR MENSAJE DE CLAIM (global — todos los grupos)
        case 'setclaimmsg':
        case 'setclaim': {
            const mensaje = args.join(' ');
            if (!mensaje) return sock.sendMessage(jid, { text: `${ERR} Uso: #setclaim [mensaje]\nUsa *{nombre}* para el personaje y *{usuario}* para tu nombre` }, Q);
            u.claimMsg = mensaje;
            usuarios[senderJid] = u;
            guardarUsuarios(usuarios);
            await sock.sendMessage(jid, { text: `${OK} Mensaje de claim global establecido:\n_${mensaje}_` }, Q);
            break;
        }

        // 🔄 RESETEAR MENSAJE DE CLAIM (global)
        case 'delclaimmsg': {
            u.claimMsg = null;
            usuarios[senderJid] = u;
            guardarUsuarios(usuarios);
            await sock.sendMessage(jid, { text: `${OK} Mensaje de claim global restablecido al predeterminado.` }, Q);
            break;
        }

        // 💬 SETEAR MENSAJE DE CLAIM PARA ESTE GRUPO
        case 'setmessageclaim':
        case 'setgroupclaim': {
            const mensaje = args.join(' ');
            if (!mensaje) {
                const actual = u.claimMsgGrupo?.[groupId];
                const ayuda =
                    `${ERR} Uso: *#setmessageclaim* [mensaje]\n\n` +
                    `Placeholders disponibles:\n` +
                    `  *{usuario}* — tu nombre\n` +
                    `  *{nombre}*  — nombre del personaje\n\n` +
                    `Ejemplo:\n_¡{usuario} ha reclamado a {nombre}!_\n\n` +
                    (actual ? `Mensaje actual en este grupo:\n_${actual}_` : `No tienes mensaje personalizado en este grupo.`);
                return sock.sendMessage(jid, { text: ayuda }, Q);
            }
            if (!u.claimMsgGrupo) u.claimMsgGrupo = {};
            u.claimMsgGrupo[groupId] = mensaje;
            usuarios[senderJid] = u;
            guardarUsuarios(usuarios);
            await sock.sendMessage(jid, {
                text: `${OK} Mensaje de claim para *este grupo* establecido:\n_${mensaje}_\n\n💡 Usa *#delmessageclaim* para quitarlo.`
            }, Q);
            break;
        }

        // 🔄 RESETEAR MENSAJE DE CLAIM DE ESTE GRUPO
        case 'delmessageclaim':
        case 'delgroupclaim': {
            if (!u.claimMsgGrupo?.[groupId]) {
                return sock.sendMessage(jid, { text: `${INFO} No tienes mensaje personalizado en este grupo.` }, Q);
            }
            delete u.claimMsgGrupo[groupId];
            usuarios[senderJid] = u;
            guardarUsuarios(usuarios);
            await sock.sendMessage(jid, { text: `${OK} Mensaje de claim de este grupo eliminado. Se usará el predeterminado.` }, Q);
            break;
        }
        ...
        case 'wvideo': {
            const nombre = args.join(' ');
            if (!nombre) return sock.sendMessage(jid, { text: `${ERR} Uso: #charvideo [nombre]` }, Q);
            // BUG 3 FIX: coincidencia exacta primero, luego parcial como fallback
            const personaje = lista.find(p => p.nombre.toLowerCase() === nombre.toLowerCase())
                           || lista.find(p => p.nombre.toLowerCase().includes(nombre.toLowerCase()));
            if (!personaje) return sock.sendMessage(jid, { text: `${ERR} Personaje *${nombre}* no encontrado` }, Q);

            await sock.sendMessage(jid, { text: `${INFO} Buscando video/gif de *${personaje.nombre}*...` }, Q);
            const mediaUrl = await buscarVideoPersonaje(personaje.tag);
            if (!mediaUrl) {
                return sock.sendMessage(jid, {
                    text: `${ERR} No encontré video/gif para *${personaje.nombre}* (${personaje.serie}).`
                }, Q);
            }
            const captionVideo = `◈ *${personaje.nombre}*\n${FS} Serie: ${personaje.serie}`;
        ...
        case 'darharem': {
            ...
            if (!isOwnerAdd(senderJid)) {
                return sock.sendMessage(jid, { text: `${ERR} Solo los owners pueden usar este comando.` }, Q);
            }
            const mencionadosAdd = (msg.message.extendedTextMessage?.contextInfo?.mentionedJid || [])
                .map(j => resolverJid(j.replace(/:\d+@/, '@')))
                .filter(j => j && !j.endsWith('@g.us') && !j.endsWith('@broadcast'));
            if (!mencionadosAdd.length) {
                return sock.sendMessage(jid, { text: `${ERR} Uso: *#addchar [nombre del personaje] @usuario*` }, Q);
            }
            ...
            if (!personajeAdd) {
                return sock.sendMessage(jid, { text: `${ERR} Personaje *${charNombreAdd}* no encontrado en la lista del bot.` }, Q);
            }
            ...
            if (yaTiene) {
                return sock.sendMessage(jid, {
                    text: `${WARN} \`${targetAddJid.split('@')[0]}\` ya tiene a *${personajeAdd.nombre}* en su harem.`
                }, Q);
            }
            ...
            await sock.sendMessage(jid, {
                text: `${OK} *${personajeAdd.nombre}* (_${personajeAdd.serie}_) fue agregado al harem de \`${targetAddJid.split('@')[0]}\`.`
            }, Q);
            break;
        }
        ...
        case 'delchar': {
            ...
            if (!mencionadosRC.length) {
                return sock.sendMessage(jid, { text: `${ERR} Uso: *#removechar [nombre del personaje] @usuario*` }, Q);
            }
            ...
            if (idxRC === -1) {
                return sock.sendMessage(jid, {
                    text: `${ERR} \`${targetRC.split('@')[0]}\` no tiene a *${charNombreRC}* en su harem.`
                }, Q);
            }
            ...
            await sock.sendMessage(jid, {
                text: `${OK} *${removedChar.nombre}* (_${removedChar.serie}_) fue eliminado del harem de \`${targetRC.split('@')[0]}\`.`
            }, Q);
            break;
        }
        ...
        case 'clearharem': {
            ...
            if (!mencionadosDel.length) {
                return sock.sendMessage(jid, { text: `${ERR} Uso: *#deleteharem @usuario*\nEscribe el comando de nuevo en 60 segundos para confirmar.` }, Q);
            }
            ...
            await sock.sendMessage(jid, {
                text: `${OK} *Harem eliminado*\n\n${FC} Usuario: \`${targetDelJid.split('@')[0]}\`\n${FI} Personajes borrados: *${haremGCount + haremLegCount}* (${haremGCount} grupo · ${haremLegCount} global)`
            }, Q);
            break;
        }
        ...
        case 'fixdupe': {
            if (!getIsOwner()(senderJid)) {
                return sock.sendMessage(jid, { text: `${ERR} Este comando es solo para owners.` }, Q);
            }
            ...
            if (!duplicados.length) {
                return sock.sendMessage(jid, {
                    text: `${OK} No hay personajes con nombre duplicado. Todo está en orden.`
                }, Q);
            }
            ...
            let texto = `${H('Nombres Duplicados')}\n`;
            ...
            pag.forEach(({ personaje: p, sugerencia, indice }, i) => {
                ...
                texto += `   ${FS} _${p.serie}_\n`;
                texto += `   ID: \`${p.id || 'sin_id'}\`\n`;
                texto += `   ${INFO} Sugerencia: *${sugerencia}*\n\n`;
            });
        ...
        // 🦹 ROB WAIFU — robarle un personaje a otro usuario
        case 'robwaifu':
        case 'robar':
        case 'steal': {
            if (groupId === 'dm') {
                return sock.sendMessage(jid, { text: `${WARN} Este comando solo funciona en grupos.` }, Q);
            }
            const mencionadosRob = (msg.message.extendedTextMessage?.contextInfo?.mentionedJid || [])
                .map(j => resolverJid(j.replace(/:\d+@/, '@')))
                .filter(j => j && !j.endsWith('@g.us') && !j.endsWith('@broadcast'));
            if (!mencionadosRob.length) {
                return sock.sendMessage(jid, { text: `${ERR} Uso: #robwaifu @usuario [nombre]\nEjemplo: #robwaifu @amigo Rem` }, Q);
            }
            const victimJid = mencionadosRob[0];
            if (victimJid === senderJid) {
                return sock.sendMessage(jid, { text: `${ERR} No puedes robarte a ti mismo.` }, Q);
            }
            const nombreRob = args.filter(a => !a.startsWith('@')).join(' ').trim();
            if (!nombreRob) {
                return sock.sendMessage(jid, { text: `${ERR} Indica el nombre del personaje a robar.\nUso: #robwaifu @usuario [nombre]` }, Q);
            }

            // Cooldown de 30 minutos para rob
            const ROB_COOLDOWN = 30 * 60 * 1000;
            const ultimoRob = u.cooldowns.robwaifu || 0;
            if (ahora - ultimoRob < ROB_COOLDOWN) {
                const restante = ROB_COOLDOWN - (ahora - ultimoRob);
                return sock.sendMessage(jid, {
                    text: `${WARN} Espera *${msToTime(restante)}* antes de volver a robar.\n${barraCooldown(restante, ROB_COOLDOWN)}`
                }, Q);
            }

            const { u: victimU, usuarios: usersRob } = getUsuarioPersonajes(victimJid);
            const victimHarem = getHaremGrupo(victimU, groupId);
            const idxRob = victimHarem.findIndex(h =>
                (typeof h === 'string' ? h : h.nombre)?.toLowerCase() === nombreRob.toLowerCase()
            );
            if (idxRob === -1) {
                return sock.sendMessage(jid, {
                    text: `${ERR} \`${victimJid.split('@')[0]}\` no tiene a *${nombreRob}* en su harem.`
                }, Q);
            }

            // Costo base: 2× el valor del personaje
            const personajeRob = lista.find(p => p.nombre.toLowerCase() === nombreRob.toLowerCase()) || { valor: 200 };
            const costoRobo = personajeRob.valor * CLAIM_COST_MULTIPLIER * 2;
            if (u.monedas < costoRobo) {
                return sock.sendMessage(jid, {
                    text: `${ERR} Necesitas *${costoRobo.toLocaleString()} coins* para intentar el robo y tienes *${u.monedas.toLocaleString()}*.\n_Gana coins con *#diario*, *#work* o *#crime*._`
                }, Q);
            }

            // 40% de éxito — el personaje puede defenderse
            const exito = Math.random() < 0.40;

            guardarUsuarioSeguro(senderJid, (uGuardado) => {
                uGuardado.monedas -= costoRobo;
                uGuardado.cooldowns.robwaifu = ahora;
                if (exito) {
                    const robado = victimHarem.splice(idxRob, 1)[0];
                    victimU.haremGrupo[groupId] = victimHarem;
                    usersRob[victimJid] = victimU;
                    getHaremGrupo(uGuardado, groupId).push(
                        typeof robado === 'object' ? robado : normalizarPersonajeHarem(personajeRob)
                    );
                }
            });
            if (exito) {
                const usersUpdated = cargarUsuarios();
                usersUpdated[victimJid] = victimU;
                guardarUsuarios(usersUpdated);
            }

            if (exito) {
                await sock.sendMessage(jid, {
                    text: `${OK} \`${pushName}\` le robó *${nombreRob}* a \`${victimJid.split('@')[0]}\`\n${FE} Costo del robo: *${costoRobo.toLocaleString()} coins*`
                }, Q);
            } else {
                await sock.sendMessage(jid, {
                    text: `${ERR} *${nombreRob}* se resistió y el robo falló.\n${FE} Perdiste *${costoRobo.toLocaleString()} coins* en el intento.`
                }, Q);
            }
            break;
        }
        ...
        try {
            await sock.sendMessage(jid, { text: `${ERR} Ocurrió un error procesando el comando. Intenta de nuevo.` }, Q);
        } catch {}
    }
}
