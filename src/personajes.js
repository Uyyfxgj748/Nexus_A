...
//
//  Cascada:  Nombre+Serie → Tag → Nombre → Variaciones → Danbooru → Gelbooru NSFW → null
...
async function buscarImagenPersonajeConFallback(personaje) {
    ...
    // Nivel 0: Nombre + Serie como dos tags booru separados
    // Da más precisión en personajes con nombres ambiguos (ej: "Rem") y
    // amplía el pool de imágenes usando el contexto de la serie
    if (nombre && serie) {
        const nombreTag = nombre.toLowerCase().replace(/\s+/g, '_').replace(/[^\w_]/g, '');
        const serieTag  = serieATagBooru(serie);
        if (nombreTag && serieTag) {
            intentos++;
            const queryNS = `${nombreTag} ${serieTag}`;
            url = await buscarPersonajeMultiTag(queryNS);
            if (url) metodo = `nombre+serie (${queryNS})`;
        }
    }
    ...
    if (url) {
        console.log(`[Personaje] ✅ "${nombre}" (${serie}) → imagen vía ${metodo} (${intentos} intento(s))`);
    } else {
        console.log(`[Personaje] ❌ "${nombre}" (${serie}) → sin imagen tras ${intentos} intento(s)`);
    }
...
function formatCaption(p, groupId) {
    const generoIcon = p.genero?.toLowerCase() === 'mujer' ? '♀' : p.genero?.toLowerCase() === 'hombre' ? '♂' : '⚧';
...
async function manejarMensajePersonajes(sock, msg) {
    ...
    switch (cmd) {
        ...
        case 'rollwaifu': {
            ...
            await sock.sendMessage(jid, { text: `${INFO} Rolleando...` }, Q);
        ...
        case 'reclamar': {
            ...
            const claimMsg = uFresco.claimMsg
                ? uFresco.claimMsg.replace('{nombre}', personaje.nombre)
        ...
        case 'wimage': {
            ...
            const imgUrl = await buscarImagenPersonajeConFallback(personaje);
            if (imgUrl) return sock.sendMessage(jid, { image: { url: imgUrl }, caption: `◈ *${personaje.nombre}* — ${personaje.serie}` }, Q);
        ...
        case 'wtop': {
            ...
            top.forEach((p, i) => {
                ...
                texto += `${i + 1}. ◈ *${p.nombre}* ${genIco} _(${p.serie})_ — ✦ ${p.valor} | ${(p.valor * CLAIM_COST_MULTIPLIER).toLocaleString()} coins\n`;
            });
            await sock.sendMessage(jid, { text: texto }, Q);
            break;
        }
        ...
        case 'animeinfo': {
            ...
            let texto = `${H('Serie Info')}\n\n${FS} *${serie}*\n${FI} Personajes: *${personajesSerie.length}*`;
        ...
        case 'animelist': {
            ...
            texto += `\n${FI} Total: *${lista.length}* personajes en *${series.length}* series`;
        ...
        case 'setclaim': {
            ...
            usuarios[senderJid] = u;
            guardarUsuarios(usuarios);
            await sock.sendMessage(jid, { text: `${OK} Mensaje de claim establecido:\n_${mensaje}_` }, Q);
            break;
        }
        ...
        case 'delclaimmsg': {
            ...
            usuarios[senderJid] = u;
            guardarUsuarios(usuarios);
            await sock.sendMessage(jid, { text: `${OK} Mensaje de claim restablecido al predeterminado.` }, Q);
            break;
        }
