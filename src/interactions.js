...
async function cmdInteraccion(sock, jid, senderJid, accion, mencionados, pushName) {
    ...
    // Intento 1: Tenor via api.delirius.store — envía por URL (más rápido, sin descargar buffer)
    try {
        const tenorRes = await axios.get(
            `https://api.delirius.store/search/tenor?q=anime+${encodeURIComponent(config.nekos)}`,
            { timeout: 8000 }
        );
        const gifs = tenorRes.data?.data;
        if (Array.isArray(gifs) && gifs.length > 0) {
            const gifUrl = gifs[Math.floor(Math.random() * gifs.length)].mp4;
            if (gifUrl) {
    ...
    } catch { /* Tenor falló — intentar con fuentes locales */ }

    // Intento 2: descarga buffer desde APIs múltiples (waifu.pics, purrbot, hmtai, safebooru, etc.)
    try {
        ...
        // Último fallback: solo texto si todo falla
