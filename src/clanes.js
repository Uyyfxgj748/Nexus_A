...
async function cmdUnirClan(sock, jid, senderJid, args) {
    ...
    // Si el clan está marcado como abierto, ingreso directo
    ...
    // Clan cerrado — sistema de solicitudes
    if (!clan.solicitudes) clan.solicitudes = [];
    if (clan.solicitudes.includes(senderJid)) {
        await sock.sendMessage(jid, { text: `⏳ Ya tienes una solicitud pendiente en *${clan.nombre}*. Espera a que el líder la revise.` });
        return;
    }
    ...
    await sock.sendMessage(jid, {
        text: `📨 *Solicitud enviada a ${clan.nombre}*\n\n_El líder del clan recibirá tu solicitud y podrá aceptarla con *#guildaccept @ti*._\n⏳ Espera la respuesta del líder.`
    });

    // Notificar al líder si está en el mismo chat
    try {
        await sock.sendMessage(jid, {
            text: `📬 @${clan.lider.split('@')[0]}, *@${senderJid.split('@')[0]}* quiere unirse a *${clan.nombre}*.\n_Usa *#guildpending* para ver solicitudes o *#guildaccept @usuario* para aprobar._`,
            mentions: [clan.lider, senderJid]
        });
    } catch {}
}
...
async function cmdVerSolicitudes(sock, jid, senderJid) {
    ...
    if (!esRangoClan(clan, senderJid)) {
        await sock.sendMessage(jid, { text: '⛔ Solo el líder o los oficiales pueden ver las solicitudes.' });
        return;
    }
    const solicitudes = clan.solicitudes || [];
    if (!solicitudes.length) {
        await sock.sendMessage(jid, { text: `📭 No hay solicitudes pendientes para *${clan.nombre}*.` });
        return;
    }
    const lista = solicitudes.map((jidS, i) => `${i + 1}. @${jidS.split('@')[0]}`).join('\n');
    await sock.sendMessage(jid, {
        text: `📋 *Solicitudes pendientes — ${clan.nombre}*\n\n${lista}\n\n_Acepta con *#guildaccept @usuario*\nRechaza con *#guilddeny @usuario*_`,
        mentions: solicitudes
    });
}
...
async function cmdGestionarSolicitud(sock, jid, senderJid, mencionados, aceptar) {
    ...
    if (!esRangoClan(clan, senderJid)) {
        await sock.sendMessage(jid, { text: `⛔ Solo el líder o los oficiales pueden ${aceptar ? 'aceptar' : 'rechazar'} solicitudes.` });
        return;
    }
    if (!mencionados || !mencionados.length) {
        await sock.sendMessage(jid, { text: `❌ Uso: *#${aceptar ? 'guildaccept' : 'guilddeny'} @usuario*` });
        return;
    }
    ...
    if (solIdx === -1) {
        await sock.sendMessage(jid, {
            text: `❌ @${targetJid.split('@')[0]} no tiene una solicitud pendiente en *${clan.nombre}*.`,
            mentions: [targetJid]
        });
        return;
    }
    ...
    if (!aceptar) {
        guardarClanes(clanes);
        await sock.sendMessage(jid, {
            text: `❌ Solicitud de @${targetJid.split('@')[0]} rechazada.`,
            mentions: [targetJid]
        });
        return;
    }

    // Aceptar — verificar que aún no esté en otro clan y que haya espacio
    const uTarget = getUsuario(targetJid);
    if (uTarget.clanId) {
        guardarClanes(clanes);
        await sock.sendMessage(jid, {
            text: `⚠️ @${targetJid.split('@')[0]} ya pertenece a otro clan. Solicitud eliminada.`,
            mentions: [targetJid]
        });
        return;
    }
    if (clan.miembros.length >= 20) {
        guardarClanes(clanes);
        await sock.sendMessage(jid, { text: `❌ El clan está lleno (20/20). No se puede aceptar a @${targetJid.split('@')[0]}.`, mentions: [targetJid] });
        return;
    }
    ...
    await sock.sendMessage(jid, {
        text: `✅ *@${targetJid.split('@')[0]}* fue aceptado en *${clan.nombre}*.\n\n👥 Miembros: *${clan.miembros.length}/20*`,
        mentions: [targetJid]
    });
}
