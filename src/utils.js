const { getGrupo, getUsuario } = require('./database');
...
async function cmdPing(sock, jid) {
    ...
    try {
        await sock.sendMessage(jid, { text: texto, edit: sent.key });
    } catch {
...
async function cmdFotoPerfil(sock, jid, senderJid, mencionados, msg) {
    ...
    try {
        // Timeout de 8s + JID normalizado
        const url = await Promise.race([
            sock.profilePictureUrl(objetivoNorm, 'image'),
            new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 8000))
        ]);
        // Descargar buffer (más fiable que pasar URL directamente a WA)
        const { data } = await axios.get(url, { responseType: 'arraybuffer', timeout: 12000 });
        const nombre = getUsuario(objetivo)?.pushName || objetivo.split('@')[0];
        ...
        await sock.sendMessage(jid, {
            text: `❌ No se pudo obtener la foto de perfil de @${objetivo.split('@')[0]}.\n_Puede que la tenga privada o que no tenga foto._`,
            mentions: [objetivo]
        });
    }
}
...
// ── Utilidades compartidas (exportadas para reutilizar en varios módulos) ──

/**
 * Loggea errores de petición HTTP suprimiendo los 429 / rate-limit
 * para no llenar el log con spam de APIs throttled.
 */
...
/**
 * Codifica tags de booru correctamente para URLs:
 * normaliza espacios, recorta y aplica encodeURIComponent.
 */
...
/**
 * Reintenta una función async ante fallos transitorios.
 * @param {Function} fn           - Función a reintentar
 * @param {number}   intentos     - Número de intentos (default 2)
 * @param {number}   delayMs      - Delay base en ms (default 1000, se multiplica por intento)
 * @param {Function} [esRateLimitFn] - Callback opcional; si devuelve true, lanza inmediatamente
 */
async function conReintentos(fn, intentos = 2, delayMs = 1000, esRateLimitFn = null) {
    let ultimoErr;
    for (let i = 0; i < intentos; i++) {
        try { return await fn(); } catch (e) {
            ultimoErr = e;
            if (esRateLimitFn && esRateLimitFn(e)) throw e;
...
module.exports = {
    ...
    logRequestError, encodeBooruTags, conReintentos
};
