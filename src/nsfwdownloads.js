...
// ── Importar helpers yt-dlp desde downloads.js (fuente única de verdad) ──
// Esto soluciona la duplicación D-03 / C-05 — el EJS solver fix queda en un solo lugar.
const {
    ytdlpEjecutar,
    ytdlpHeadersArgs,
    extraerErrorYtdlp,
...
// Descarga un vídeo NSFW a buffer — timeout largo (180 s) por tamaño de archivos
...
//  XVIDEOS (de Yotsuba)
// ════════════════════════════════════════════════════
async function cmdXvideos(sock, jid, args) {
    const url = (args[0] || '').trim();
    if (!url || !/xvideos\.com/.test(url)) {
        await sock.sendMessage(jid, { text: '❌ Uso: *#xvideos <link>*\nEjemplo: *#xvideos https://www.xvideos.com/video...*' });
        return;
    }
    await sock.sendMessage(jid, { text: '🔞 Descargando vídeo de *XVideos*... ⏳' });
    try {
        const info = await ytdlpInfo(url);
        const buffer = await ytdlpDescargarVideo(url, 'xvideos');
        const titulo = info.title ? `🔞 *XVideos*\n🎬 ${info.title}` : '🔞 *XVideos*';
        await sock.sendMessage(jid, { video: buffer, mimetype: 'video/mp4', caption: titulo });
    } catch (err) {
        logRequestError('cmdXvideos', err);
        await sock.sendMessage(jid, { text: `❌ Error XVideos: ${extraerErrorYtdlp(err)}` });
    }
}

const TODO_NSFW_DOWNLOADS = ['hitomila', 'hitomi', 'nhentai', 'nh', 'nhdl', 'vermangasporno', 'vmp', 'xnxx', 'pornhub', 'ph', 'xvideos', 'xvid'];
