...
const { H, SH, F, FS, FI, FC, FE, OK, ERR, WARN, INFO, barra } = require('./style');
...
function getDiaNombre()    { return Math.floor(Date.now() / 86400000); }
function getSemanaNumero() { return Math.floor(Date.now() / (86400000 * 7)); }
...
async function cmdMisiones(sock, jid, senderJid) {
    ...
    let txt = `${H('Misiones del dia')}\n${FI} Expiran en *${mañana.toFixed(1)}h*\n\n`;
    lista.forEach((id, i) => {
        ...
        const bar        = barra(progreso, m.meta, 8);
        const estado     = reclamada ? '_Reclamada_' : completada ? '*Lista para reclamar*' : `[${bar}] ${progreso}/${m.meta}`;
        txt += `${i + 1}. ${reclamada ? '~~' : ''}*${m.texto}*${reclamada ? '~~' : ''}\n`;
        txt += `${FE} *${m.premio.toLocaleString()} ⓃNC* — ${estado}\n\n`;
    });
    txt += `_#claimmission para reclamar  ·  #weeklymissions para misiones semanales_`;
...
async function cmdClaimMision(sock, jid, senderJid) {
    ...
    if (!totalGanado) {
        await sock.sendMessage(jid, { text: `${ERR} No tienes misiones completadas para reclamar.\n_Usa *#missions* para ver tu progreso._` });
        return;
    }
    ...
    const listaTxt = nuevas.map(t => `${OK} _${t}_`).join('\n');
    await sock.sendMessage(jid, {
        text: `${H('Misiones reclamadas')}\n\n${listaTxt}\n\n${FE} Total ganado » *+${totalGanado.toLocaleString()} ⓃNexCoins*\n${F} Saldo » *${u.monedas.toLocaleString()} ⓃNC*`
    });
}
...
async function cmdMisionesSemanales(sock, jid, senderJid) {
    ...
    let txt = `${H('Misiones Semanales')}\n${FI} Expiran en *${diasRestantes} días*\n_Mayor dificultad, mayores premios._\n\n`;
    lista.forEach((id, i) => {
    ...
    txt += `_#claimweekly para reclamar misiones semanales completadas._`;
    await sock.sendMessage(jid, { text: txt });
}
...
async function cmdClaimMisionSemanal(sock, jid, senderJid) {
    ...
    if (!totalGanado) {
        await sock.sendMessage(jid, { text: `${ERR} No tienes misiones semanales completadas para reclamar.\n_Usa *#weeklymissions* para ver tu progreso._` });
        return;
    }
    ...
    const listaTxt = nuevas.map(t => `${OK} _${t}_`).join('\n');
    await sock.sendMessage(jid, {
        text: `${H('Misiones Semanales Reclamadas')}\n\n${listaTxt}\n\n${FE} Total ganado » *+${totalGanado.toLocaleString()} ⓃNexCoins*\n${F} Saldo » *${u.monedas.toLocaleString()} ⓃNC*\n\n_Las misiones se renuevan cada lunes._`
    });
}
