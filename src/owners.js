...
function guardarOwners(lista) {
    ...
    _ownersCache = lista; // invalidar y actualizar caché
}
...
function isSuperOwner(jid) {
    if (!jid) return false;
    // Comparar contra el SUPER_OWNER hardcoded
    if (_coincide(jid, SUPER_OWNER)) return true;
...
function removeOwner(jid) {
    if (isSuperOwner(jid)) return false;
    const owners = cargarOwners();
    const jidNorm = normalizarJid(jid);
