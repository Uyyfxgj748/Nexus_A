...
const { cmdTrivia, cmdMath, cmdGuess, cmdWordchain, cmdStopGame, cmdPpt, cmdAhorcado, cmdScramble, cmdQuien, procesarRespuesta, cmdTsQuiz, cmdCompleta, cmdVof, cmdEmojiAdivina, cmdBola8, cmdTictactoe, cmdMovTtt, cmdAbandonarTtt, cmdSopa } = require('./minijuegos');
...
const { cmdBrat, cmdBrat2, cmdBrat3, cmdTtp, cmdAttp, cmdQc, cmdWm } = require('./stickertext');
const { cmdConverPDF } = require('./converters');
...
const {
    esPinUrl, descargarPin, buscarPinterest, buscarPinsDirecto, formatearInfoPin, fetchPinInfo,
...
const { cmdTranslate, cmdWikipedia, cmdSsweb, cmdIpLookup, cmdCalc, cmdQr, cmdManga,
    cmdEmojimix, cmdTempmail, cmdInspectGrupo, cmdNpmjs, cmdShazam } = require('./utiltools');
const { cmdEfectoAudio, TODO_EFECTOS_AUDIO } = require('./audioefectos');
...
const {
    ...
    cmdTopWeekly, cmdTopSemanalResultados
} = require('./profile');
...
const {
    ...
    cmdSetTagMode, cmdConfig, cmdAnticall
} = require('./admin');
...
const rateLimitConfig = new Map([
    ...
    ['ttfotos', { window: 60 * 1000, max: 2, lock: 5 * 60 * 1000 }],
...
async function _manejarBusquedaPin(sock, from, query) {
    ...
    let pool = [];
    try {
        pool = await buscarPinsDirecto(query);
    } catch (e) {
...
async function manejarMensaje(sock, msg, groupMetadata) {
    ...
    // Un Sub-Bot puede activar/desactivar SOLO el chat donde está presente,
    // usando su propia sesión (fromMe) — nunca chats ajenos. Así el dueño de
    // un sub-bot no depende del SUPER_OWNER para usarlo en sus propios grupos.
    const puedeActivarComoSubbot = sock.__esSubbot && msg.key.fromMe;

    if (textoLower === '#activatebot' || textoLower === '#enablebot') {
        if (isSuperOwner(senderJid) || puedeActivarComoSubbot) {
    ...
    // ── Sub-Bots ─────────────────────────────────────────────────────────
    // Viene desactivado por defecto; solo un owner puede activarlo/desactivarlo.
    // Cualquier usuario puede vincular su número con #serbot mientras esté activo.
    if (textoLower.startsWith('#subbots') || textoLower.startsWith('#subbot')) {
        const { getSubbotsActivo, setSubbotsActivo, listarSubBots, listarSubBotsDetallado } = require('./subbots');
        const partes = texto.trim().split(/\s+/).slice(1);
        const sub = (partes[0] || '').toLowerCase();

        if (sub === 'on' || sub === 'off') {
            if (!isOwner(senderJid)) {
                await sock.sendMessage(jid, { text: `${ERR} Solo los owners pueden activar o desactivar los Sub-Bots.` });
                return;
            }
            const activo = setSubbotsActivo(sub === 'on');
            await sock.sendMessage(jid, {
                text: activo
                    ? `${OK} *Sub-Bots activados.*\nCualquier usuario puede vincular su número como sub-bot con *#serbot*.`
                    : `${INFO} *Sub-Bots desactivados.*\nSe cerraron todas las sesiones de sub-bots que estaban activas.`
            });
            return;
        }

        // Los owners ven el listado completo (nombre, número, dueño, uso).
        // Cualquier otro usuario solo ve el estado general, sin números ni dueños.
        if (isOwner(senderJid)) {
            const detalle = listarSubBotsDetallado();
            const fmtFecha = ts => ts ? new Date(ts).toLocaleString('es-ES') : '—';
            const lineas = detalle.length
                ? detalle.map((s, i) => {
                    const dueño = s.dueñoJid ? `@${s.dueñoJid.split('@')[0]}` : 'desconocido';
                    return [
                        `*${i + 1}. +${s.numero}* — ${s.conectado ? 'conectado ✅' : 'desconectado ⛔'}`,
                        `   Nombre: ${s.nombre || '—'}`,
                        `   Dueño: ${dueño}`,
                        `   Vinculado: ${fmtFecha(s.vinculadoEn)}`,
                        `   Comandos ejecutados: ${s.comandosEjecutados} ${s.ultimoComando ? `(último: #${s.ultimoComando}, ${fmtFecha(s.ultimoUso)})` : ''}`
                    ].join('\n');
                }).join('\n\n')
                : '_Ningún sub-bot registrado._';
            await sock.sendMessage(jid, {
                text: `${H('SUB-BOTS (owner)')}\nEstado global: ${getSubbotsActivo() ? 'activado ✅' : 'desactivado ⛔'}\n\n${lineas}\n\n_*#subbots on/off* — activar o desactivar_\n_*#delsubbot <número>* — desvincular uno a la fuerza_`,
                mentions: detalle.filter(s => s.dueñoJid).map(s => s.dueñoJid)
            });
            return;
        }
        ...
        await sock.sendMessage(jid, {
            text: `${H('SUB-BOTS')}\nEstado: ${getSubbotsActivo() ? 'activado ✅' : 'desactivado ⛔'}\nSub-bots conectados: ${activos.filter(s => s.conectado).length}\n\n_Usa *#serbot* para vincular tu número (si está activado)._\n_Usa *#delbot* desde tu sub-bot para desvincularlo cuando quieras._`
        });
        return;
    }

    if (textoLower === '#serbot' || textoLower.startsWith('#serbot ') || textoLower === '#code') {
        const { getSubbotsActivo, iniciarSubBot, verificarCooldown, marcarCooldown, listarSubBots, getSubbotDeDueño, MAX_SUBBOTS } = require('./subbots');

        if (!getSubbotsActivo()) {
            await sock.sendMessage(jid, { text: `${ERR} La función de *Sub-Bots* está desactivada.\n_Un owner puede activarla con *#subbots on*._` });
            return;
        }

        const restante = verificarCooldown(senderJid);
        if (restante > 0) {
            await sock.sendMessage(jid, { text: `${WARN} Espera ${Math.ceil(restante / 1000)}s antes de volver a solicitar un código.` });
            return;
        }

        const yaVinculado = getSubbotDeDueño(senderJid);
        if (yaVinculado) {
            await sock.sendMessage(jid, { text: `${ERR} Ya tienes un Sub-Bot vinculado (*+${yaVinculado}*).\n_Usa *#delbot* desde esa sesión para desvincularlo antes de crear otro._` });
            return;
        }

        if (listarSubBots().length >= MAX_SUBBOTS) {
            await sock.sendMessage(jid, { text: `${ERR} No hay espacios disponibles para nuevos Sub-Bots en este momento.` });
            return;
        }

        const argNumero = texto.trim().split(/\s+/)[1];
        const numero = (argNumero ? argNumero : senderJid.split('@')[0]).replace(/\D/g, '');
        if (!numero || numero.length < 8) {
            await sock.sendMessage(jid, { text: `${ERR} Número inválido.\nUso: *#serbot* (vincula tu propio número) o *#serbot <número con código de país>*` });
            return;
        }

        marcarCooldown(senderJid);
        // El código se envía siempre por privado al solicitante, nunca al chat
        // donde se escribió el comando — así nadie más en un grupo puede verlo.
        if (jid !== senderJid) {
            await sock.sendMessage(jid, { text: `${INFO} Te envío el código de vinculación por privado.` });
        }
        await sock.sendMessage(senderJid, { text: `${INFO} Generando código de vinculación para *+${numero}*...` }).catch(() => {});
        try {
            await iniciarSubBot({ numero, sockPrincipal: sock, chatId: senderJid, dueñoJid: senderJid });
        } catch (e) {
            await sock.sendMessage(senderJid, { text: `${ERR} ${e.message}` }).catch(() => {});
        }
        return;
    }

    // ── #delbot — el dueño de un Sub-Bot lo desvincula desde su propia sesión ─
    if (textoLower === '#delbot' || textoLower === '#dejardebot' || textoLower === '#unsubbot') {
        if (!sock.__esSubbot) {
            await sock.sendMessage(jid, { text: `${ERR} Este comando solo funciona en la sesión de un Sub-Bot.\n_Envíalo desde el número que vinculaste con *#serbot*._` });
            return;
        }
        if (!msg.key.fromMe) {
            await sock.sendMessage(jid, { text: `${ERR} Solo el dueño de este Sub-Bot puede desvincularlo.` });
            return;
        }
        const { detenerSubBot } = require('./subbots');
        const numero = sock.__subbotNumero;
        await sock.sendMessage(jid, { text: `👋 Desvinculando este Sub-Bot y borrando su sesión...` });
        setTimeout(() => {
            detenerSubBot(numero, { eliminarSesion: true, eliminarRegistro: true }).catch(() => {});
        }, 1200);
        return;
    }

    // ── #delsubbot <número> — un owner fuerza la desvinculación de cualquiera ─
    if (textoLower.startsWith('#delsubbot')) {
        if (!isOwner(senderJid)) {
            await sock.sendMessage(jid, { text: `${ERR} Solo los owners pueden forzar la desvinculación de un Sub-Bot.` });
            return;
        }
        const numeroArg = (texto.trim().split(/\s+/)[1] || '').replace(/\D/g, '');
        if (!numeroArg) {
            await sock.sendMessage(jid, { text: `${ERR} Uso: *#delsubbot <número>*\n_Consulta los números con *#subbots*._` });
            return;
        }
        const { detenerSubBot } = require('./subbots');
        const existia = await detenerSubBot(numeroArg, { eliminarSesion: true, eliminarRegistro: true });
        await sock.sendMessage(jid, {
            text: existia
                ? `${OK} Sub-Bot *+${numeroArg}* desvinculado.`
                : `${WARN} No se encontró ningún Sub-Bot conectado con ese número (puede que ya estuviera desconectado; se limpió su registro si existía).`
        });
        return;
    }
...
◇ Premio: *+${recompensa} ⓃNexCoins*`;
    ...
    const comandosConCooldownGlobal = new Set([
        ...
        'ytv', 'ytvideo', 'ytdescargar', 'tiktok', 'tt', 'ttplay', 'tiktokmp3', 'ttaudio', 'ttfotos',
    ...
    try {
        ...
        switch (cmd) {
            ...
            case 'menu': case 'ayuda': case 'help': case 'commands': case 'comandos':
                await enviarMenu(sock, jid, pushName, groupMetadata, senderJid, args.join(' ').trim() || null); break;
            ...
            case 'ssweb': case 'screenshot': case 'screenshotweb':
            ...
            case 'pfp': case 'getpic':
                await cmdFotoPerfil(sockR, jid, senderJid, mencionados, msg); break;
            ...
            case 'estadisticas': case 'botstats':
            ...
            // ── CASINO ────────────────────────────────────────────────────
            // ── TRES EN RAYA ──────────────────────────────────────────────
            case 'ttt': case 'tictactoe': case 'tresenraya': case 'gato':
                await cmdTictactoe(sockR, jid, senderJid, mencionados, pushName); break;
            case 't': case 'jugar': case 'mover': case 'mov':
                await cmdMovTtt(sockR, jid, senderJid, args); break;
            case 'tttabandonar': case 'abandottt': case 'tttrendir': case 'tttrendirse':
                await cmdAbandonarTtt(sockR, jid, senderJid); break;
            ...
            case 'topsemanalresultados': case 'semanalresultados': case 'weeklyresults': case 'topweeklyresults':
                await cmdTopSemanalResultados(sockR, jid); break;
            ...
            // case 'brat': await cmdBrat(sockR, jid, args); break;   // desactivado
            // case 'brat2': await cmdBrat2(sockR, jid, args); break; // desactivado
            // case 'brat3': await cmdBrat3(sockR, jid, args); break; // desactivado
            case 'converpdf': case 'convertpdf': case 'topdf': case 'img2pdf':
                await cmdConverPDF(sockR, jid, msg); break;
            case 'ttp':
                await cmdTtp(sockR, jid, args); break;
            case 'attp':
                await cmdAttp(sockR, jid, args); break;
            case 'qc':
                await cmdQc(sockR, jid, msg, mencionados); break;
            case 'wm':
                await cmdWm(sockR, jid, msg, args); break;
            ...
            case 'ttfotos':
                await cmdTiktokFotos(sockR, jid, args); break;
            case 'tts': case 'ttsticker': case 'tiktoksticker':
                await cmdTiktokSticker(sockR, jid, args, pushName); break;
            ...
            case 'mega':
                await cmdMega(sockR, jid, args); break;
            case 'terabox':
                await cmdTerabox(sockR, jid, args); break;
            case 'gitclone': case 'gitdl':
                await cmdGitclone(sockR, jid, args); break;
            case 'dl': case 'aio':
                await cmdUniversalDl(sockR, jid, args); break;
            case 'pixiv': case 'px':
            ...
            case 'anticall': case 'antillamada': case 'nollamadas':
                await cmdAnticall(sockR, jid, groupMetadata, senderJid, args); break;
            ...
            case 'qr': case 'generarqr': case 'makeqr': case 'leerqr': case 'readqr': case 'decodificarqr':
                await cmdQr(sockR, jid, msg, args); break;
            case 'manga': case 'buscarman': case 'searchmanga': case 'mangainfo':
                await cmdManga(sockR, jid, args); break;
            case 'emojimix': case 'emojikitchen': case 'mezclaremoji':
                await cmdEmojimix(sockR, jid, args); break;
            case 'tempmail': case 'correotemporal': case 'mailtemp': case 'disposablemail':
                await cmdTempmail(sockR, jid, senderJid, args); break;
            case 'inspect': case 'inspectgroup': case 'infolink': case 'groupinfo':
                await cmdInspectGrupo(sockR, jid, args); break;
            case 'npmjs': case 'npm': case 'buscarnpm': case 'npmsearch':
                await cmdNpmjs(sockR, jid, args); break;
            case 'shazam': case 'identificarcancion': case 'reconocercancion':
                await cmdShazam(sockR, jid, msg); break;

            // ── JUEGOS ────────────────────────────────────────────────────
            case 'sopa': case 'sopade': case 'sopadepalabras': case 'spdeletras': case 'wordsearch': case 'buscarpalabra':
                await cmdSopa(sockR, jid, senderJid); break;
            ...
            default: {
                ...
                } else if (TODO_EFECTOS_AUDIO.includes(cmd)) {
                    await cmdEfectoAudio(sockR, jid, msg, cmd);
                } else {
                    await sockR.sendMessage(jid, {
