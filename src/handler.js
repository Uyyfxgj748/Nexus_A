...
async function _manejarDescargaPin(sock, from, url) {
    await sock.sendMessage(from, { text: `⏳ Descargando pin de Pinterest...` });
    let resultado;
    try {
        ...
        if (msg.includes('private') || msg.includes('privad')) {
            await sock.sendMessage(from, { text: '⚠️ Este pin es privado y no se puede descargar.' });
        } else if (msg.includes('deleted') || msg.includes('eliminado') || msg.includes('not available') || msg.includes('404')) {
            await sock.sendMessage(from, { text: '⚠️ Este pin ya no existe o fue eliminado.' });
        } else if (msg.includes('login') || msg.includes('sign in') || msg.includes('autenticaci')) {
            await sock.sendMessage(from, { text: '⚠️ Pinterest requiere autenticación para este contenido. Actualiza las cookies del bot.' });
        } else if (msg.includes('timeout') || msg.includes('ETIMEDOUT') || msg.includes('ECONNRESET')) {
            await sock.sendMessage(from, { text: '⚠️ Tiempo de espera agotado. Intenta de nuevo en unos segundos.' });
        } else {
            await sock.sendMessage(from, { text: `❌ No se pudo descargar el pin.\n_${msg.split('\n')[0]}_` });
        }
        return;
    }

    const { tipo, buffer, info } = resultado;
    const caption = formatearInfoPin(info);

    try {
        if (tipo === 'video') {
            await sock.sendMessage(from, {
                video  : buffer,
                caption: caption || '📌 *Pinterest*',
                mimetype: 'video/mp4'
            });
        } else {
            await sock.sendMessage(from, {
        ...
        // Si el envío falla (archivo muy grande, etc.) informar al usuario
        await sock.sendMessage(from, {
            text: `❌ No se pudo enviar el archivo (puede ser demasiado grande).\n🔗 ${info.url}`
        });
    }
}

// ── Helpers para búsqueda Pinterest ───────────────────────────────────────
const _UA_PIN = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36';
...
function _buildPinCaption(query, item) {
    ...
    if (item.autor || item.username) {
        const display = (item.autor && item.username && item.autor !== item.username)
            ? `${item.autor} (${item.username})` : item.autor || item.username;
        caption += `\n❀ *Usuario »* ${display}`;
    }
    if (item.titulo)  caption += `\n❖ *Título »* ${item.titulo}`;
    if (item.tablero) caption += `\n□ *Tablero »* ${item.tablero}`;
    if (item.urlPin)  caption += `\n🔗 *Link »* ${item.urlPin}`;
    caption += `\n\n_Responde con 🔄 para otra imagen_`;
    return caption;
}

// ── Búsqueda de imágenes Pinterest (solo API oficial) ─────────────────────
async function _manejarBusquedaPin(sock, from, query) {
    const yaIndexado = require('./pinterest').tieneTokenBusqueda?.() ?? false;
    await sock.sendMessage(from, {
        text: yaIndexado
            ? `🔍 Buscando en Pinterest: *${query}*...`
            : `🔍 Buscando en Pinterest: *${query}*...\n_(primera vez — cargando tus tableros, puede tardar unos segundos)_`
    });

    const pool = await buscarPinterest(query);

    if (!pool.length) {
        await sock.sendMessage(from, {
            text: `❌ No encontré pines para: *${query}*`
        });
        return;
    }

    const usados = new Set();
    const idx    = Math.floor(Math.random() * Math.min(pool.length, 15));
    usados.add(idx);
    let item = pool[idx];

    // Intentar enriquecer con info del creador original via scrape del pin
    if (item.urlPin) {
        try {
    ...
    try {
        ...
        try {
            ...
            await sock.sendMessage(from, { text: `❌ No se pudo enviar la imagen. Intenta de nuevo.` });
            return;
        }
    }
    ...
    if (sentMsg?.key?.id) {
        ...
        if (pinMap.size > 200) {
            const firstKey = pinMap.keys().next().value;
            pinMap.delete(firstKey);
        }
    }
}
...
async function manejarPin(sock, msg, isReply = false) {
    ...
    if (isReply) {
        ...
        if (!disponibles.length) {
            await sock.sendMessage(from, { text: `🔄 Ya mostré todas las imágenes disponibles para: *${query}*\nIntenta una búsqueda nueva.` });
            return;
        }
        ...
        try {
            ...
            try {
                ...
                await sock.sendMessage(from, { text: '❌ No se pudo enviar la imagen.' });
                return;
            }
        }
    ...
    if (!raw) {
        await sock.sendMessage(from, {
            text:
                '📌 *Comando #pin — Pinterest*\n\n' +
                '• `#pin <búsqueda>` → busca pines guardados en tu cuenta\n' +
                '• `#pin <link>` → descarga imagen o vídeo de Pinterest\n\n' +
                '*Ejemplos:*\n' +
                '`#pin anime`\n' +
                '`#pin wallpaper aesthetic`\n' +
                '`#pin https://pin.it/xxxxxx`\n\n' +
                '_Responde con 🔄 a cualquier resultado para ver otro pin._'
        });
        return;
    }
...
async function cmdQuitarDinero(sock, jid, senderJid, args, mencionados) {
    ...
    if (!cantidad || cantidad <= 0) {
        await sock.sendMessage(jid, { text: '❌ Debes indicar una cantidad válida mayor a 0 o *all*.' });
        return;
    }
    ...
    if (saldoActual < cantidad) {
        await sock.sendMessage(jid, {
            text: `⚠️ @${numero} solo tiene *${saldoActual.toLocaleString()} ⓃNexCoins* en cartera (banco: *${(u.banco || 0).toLocaleString()}*).\nUsa *#quitarcoins @usuario all* para borrar todo.`,
            mentions: [targetJid]
        });
        return;
    }
...
async function manejarMensaje(sock, msg, groupMetadata) {
    ...
    if (textoLower.startsWith('#maplid')) {
        if (!isOwner(senderJid)) {
            await sock.sendMessage(jid, { text: '❌ Solo los owners pueden usar este comando.' });
            return;
        }
        ...
        if (arg2) {
            ...
            if (!isSuperOwner(senderJid)) {
                await sock.sendMessage(jid, { text: '❌ Solo el super-owner puede mapear LIDs de terceros.' });
                return;
            }
...
║  👤 *${nombre}*
${divider}
║  ${rangoAnterior.emoji} Nv. *${expRes.nivelAnterior}*  ➜  ${rangoNuevo.emoji} Nv. *${expRes.nivelNuevo}* 🏆
    ...
    try {
        ...
        switch (cmd) {
            ...
            case 'levelnotif': case 'nivelnotif': case 'levelanuncios': case 'anunciosnivel': {
                if (!esGrupo) { await sockR.sendMessage(jid, { text: '⛔ Este comando solo funciona en grupos.' }); break; }
                if (!isOwner(senderJid) && !esAdmin(groupMetadata, senderJid)) {
                    await sockR.sendMessage(jid, { text: '⛔ Solo los admins o el owner pueden cambiar esta configuración.' }); break;
                }
                ...
                if (_sub === 'on' || _sub === 'enable' || _sub === 'activar') {
                    if (_estadoActual) { await sockR.sendMessage(jid, { text: '⚠️ Los anuncios de nivel ya están *activados*.' }); break; }
                    _gNv.anunciosNivel = true;
                    guardarGrupo(jid, _gNv);
                    await sockR.sendMessage(jid, { text: '✅ *Anuncios de nivel activados.*\nCuando alguien suba de nivel, el bot lo anunciará en el grupo.' });
                } else if (_sub === 'off' || _sub === 'disable' || _sub === 'desactivar') {
                    if (!_estadoActual) { await sockR.sendMessage(jid, { text: '⚠️ Los anuncios de nivel ya están *desactivados*.' }); break; }
                    _gNv.anunciosNivel = false;
                    guardarGrupo(jid, _gNv);
                    await sockR.sendMessage(jid, { text: '🔕 *Anuncios de nivel desactivados.*\nLas subidas de nivel ya no se anunciarán en el grupo.' });
                } else {
                    const estado = _estadoActual ? '✅ *Activados*' : '🔕 *Desactivados*';
                    await sockR.sendMessage(jid, {
                        text: `📢 *Anuncios de subida de nivel*\nEstado actual: ${estado}\n\n_Usa *#levelnotif on* para activar o *#levelnotif off* para desactivar._`
                    });
                }
                break;
            }
