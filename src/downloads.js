...
// Flags globales para resolver el JavaScript challenge de YouTube (yt-dlp 2026+).
// `which node` devuelve el wrapper Nix (distinto de process.execPath que da el binario
// desnudo). El wrapper configura librerías nativas necesarias para el solver EJS;
// sin él yt-dlp reporta "node (unsupported)" y solo obtiene storyboards.
...
async function ytdlpDescargarBuffer(url, { formato = null, merge = false, cookiesPath = null, ytArgs = null, onProgress = null } = {}) {
    ...
    try {
        if (onProgress) {
            await ytdlpEjecutarConProgreso(construirArgs(ytArgs), { timeout: 150000, onProgress });
        } else {
...
async function ytdlpBuscarUrl(query) {
    ...
    const hayCookies = tieneCookiesReales();
    // Con cookies: el bot-check se evita; android,web_safari con cookies permite
    // que web_safari acceda aunque android sea saltado por no soportar cookies.
    // Sin cookies: los clientes android son intentados primero (falla en IPs bloqueadas)
    const intentos = hayCookies
        ? [
            { args: ytYoutubeArgs(),        addCookies: true  },  // android,web_safari + cookies
            { args: ytYoutubeArgsCookies(), addCookies: false },  // web_safari + cookies (ya incluye)
          ]
        : [
            { args: ytYoutubeArgsAudio(),   addCookies: false },  // mweb,android
            { args: ytYoutubeArgs(),        addCookies: false },  // android,web_safari
          ];
    let ultimoError;
...
async function twitterObtenerMedia(url) {
    ...
    // vxtwitter — detecta video Y gif, prioriza mayor calidad (variants por bitrate)
    try {
    ...
    // fxtwitter — fallback, también prioriza variant de mayor calidad
    try {
