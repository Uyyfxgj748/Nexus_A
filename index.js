...
const { getGrupo, cargarGrupos, migrarCuentasLid, limpiarDuplicadosLid, getUsuario } = require('./src/database');
...
const { procesarMensajeEntrante } = require('./src/messageRouter');
const { reconectarSesionesGuardadas } = require('./src/subbots');
    ...
    try {
        ...
        sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
            ...
            if (connection === 'open') {
                ...
                // Restaurar sub-bots que ya tenían sesión guardada (solo si la
                // función de sub-bots está activada — por defecto está apagada).
                reconectarSesionesGuardadas(sock).catch(err =>
                    console.error('[SUBBOTS] Error restaurando sesiones:', err.message)
                );
                botState.conectadoEn = Date.now();
        ...
        // Periodo de gracia al arrancar: ignorar bienvenidas/despedidas de
        // eventos backfill (acumulados mientras el bot estuvo apagado).
        // Los mensajes en tiempo real siguen funcionando con normalidad.
        ...
        sock.ev.on('group-participants.update', async ({ id, participants, action }) => {
            try {
                ...
                // Si el evento llega dentro del periodo de gracia tras el arranque
                // es backfill histórico — no mandar bienvenidas/despedidas masivas.
                const enGracia = !botState.conectadoEn || (Date.now() - botState.conectadoEn) < STARTUP_GRACE_MS;
                if (enGracia && (action === 'add' || action === 'invite' || action === 'remove')) {
                    return;
                }
                ...
                // Bordes decorativos (mismo estilo que el menú)
                const BG_TOP    = '╭┈ࠢ͜͜┅ࠦ͜͜╾݊͜─ؕ͜─ׄ͜─֬͜─֟͜─֫͜─ׄ͜─ؕ͜─݊͜┈ࠦ͜┅ࠡ͜͜┈࠭͜͜۰۰͜۰';
                const BG_BOT    = '╰ׅ┈ࠢ͜─ׄ͜─ׄ֟፝͜─ׄ͜─ׄ͜╴ ⋱࣭ ᩴ  ⋮֔   ᩴ ⋰╶͜─ׄ͜─ׄ֟፝͜─ׄ͜─ׄ͜┈ࠢ͜╯ׅ';

                // Banner del menú como fallback de imagen (si el grupo no tiene media propia)
                const { leerMenuMedia: _leerMenuMedia } = require('./src/menu');
                const menuBanner = _leerMenuMedia();
                ...
                if ((action === 'add' || action === 'invite') && g.bienvenida) {
                    ...
                    for (const p of participants) {
                        ...
                        const nombre = getUsuario(jid)?.pushName || jid.split('@')[0];

                        let texto;
                        if (g.mensajeBienvenida) {
                            // Mensaje personalizado del admin: conservar @mención estándar
                            texto = g.mensajeBienvenida
                                .replace(/@usuario|(?<!\w)@(?!\w)/g, `@${jid.split('@')[0]}`);
                        } else {
                            // Mensaje por defecto con estilo del menú
                            texto =
`${BG_TOP}
│𖧧 *ʙɪᴇɴᴠᴇɴɪᴅᴏ/ᴀ* \`${nombre}\`
│○ *ɢʀᴜᴘᴏ ::* ${groupName}
│✿ *ᴄᴏᴍᴀɴᴅᴏs ::* escribe \`#menu\` 🚀
${BG_BOT}`;
                        }

                        const mediaFinal = customMedia || menuBanner;
                        await enviarMediaBG(id, texto, jid, mediaFinal);
                    }
                }
                ...
                if (action === 'remove' && g.despedida) {
                    ...
                    for (const p of participants) {
                        ...
                        let texto;
                        if (g.mensajeDespedida) {
                            // Mensaje personalizado del admin: conservar @mención estándar
                            texto = g.mensajeDespedida
                                .replace(/@usuario|(?<!\w)@(?!\w)/g, `@${jid.split('@')[0]}`);
                        } else {
                            // Mensaje por defecto con estilo del menú
                            texto =
...
${BG_BOT}`;
        ...
        // ── Anticall: rechazar llamadas y expulsar si anticall está ON ─
        sock.ev.on('call', async (calls) => {
            for (const call of calls) {
                if (call.status !== 'offer') continue;
                // Siempre rechazar la llamada
                try { await sock.rejectCall(call.id, call.from); } catch {}
                // Verificar si el llamante está en algún grupo con anticall=true
                try {
                    const { cargarGrupos, getGrupo } = require('./src/database');
                    const grupos = cargarGrupos();
                    for (const gJid of Object.keys(grupos)) {
                        const gData = getGrupo(gJid);
                        if (!gData.anticall) continue;
                        const meta = await getGroupMeta(sock, gJid);
                        if (!meta?.participants) continue;
                        const esParticipante = meta.participants.some(p => {
                            const pid = typeof p === 'string' ? p : (p.id || p.jid || '');
                            return pid === call.from || pid.replace(/:\d+@/, '@') === call.from;
                        });
                        if (esParticipante) {
                            try {
                                await sock.groupParticipantsUpdate(gJid, [call.from], 'remove');
                                await sock.sendMessage(gJid, {
                                    text: `📵 @${call.from.split('@')[0]} fue expulsado por llamar al bot.\n_El anticall está activado en este grupo._`,
                                    mentions: [call.from]
                                });
                            } catch {}
                        }
        ...
        // ── Mensajes entrantes ─────────────────────────────────────────
        // El gate y el despacho de comandos viven en src/messageRouter.js,
        // compartido con cada socket de Sub-Bot (src/subbots.js) para que
        // ambos respeten exactamente las mismas restricciones.
        sock.ev.on('messages.upsert', async ({ messages, type }) => {
            ...
            for (const msg of messages) {
                await procesarMensajeEntrante(sock, msg);
            }
        });
...
<p style="margin-top:24px;font-size:13px;color:#aaa;">Expira: ${new Date(expiresAt * 1000).toLocaleDateString('es-ES', { day:'2-digit', month:'long', year:'numeric' })}</p>
    ...
    // ── Página real con meta Open Graph para el link preview del #menu ─────
    // WhatsApp (o el teléfono vinculado) descarga esta URL de verdad y lee
    // sus meta tags og:image para generar la tarjeta grande — por eso el
    // dominio debe servir contenido real, no un thumbnail inventado.
    if (url === '/menu-preview') {
        const dominio = process.env.REPLIT_DEV_DOMAIN || (process.env.REPLIT_DOMAINS || '').split(',')[0];
        if (!dominio) {
            res.writeHead(503, { 'Content-Type': 'text/plain; charset=utf-8' });
            res.end('❌ No hay dominio disponible para generar el preview.');
            return;
        }
        const imagenUrl = `https://${dominio}/menu-banner.jpg`;
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
        res.end(`<!DOCTYPE html><html><head><meta charset="utf-8">
<meta property="og:title" content="⚡ NEXUS•SYSTEM">
<meta property="og:description" content="Made with ❤️ by Nexus•System">
...
</head><body><img src="${imagenUrl}" alt="Nexus System"></body></html>`);
        return;
    }

    // ── Sirve el banner real que referencia la página de arriba ─────────────
    if (url === '/menu-banner.jpg') {
        try {
            const { leerMenuMedia } = require('./src/menu');
            ...
            console.error('Error sirviendo menu-banner:', e.message);
        }
        res.writeHead(404, { 'Content-Type': 'text/plain' });
        res.end('No hay imagen de menú configurada');
        return;
    }
    // ── Igual que /menu-preview pero para el banner del #menu18 (NSFW) ──────
    if (url === '/menu18-preview') {
        const dominio = process.env.REPLIT_DEV_DOMAIN || (process.env.REPLIT_DOMAINS || '').split(',')[0];
        if (!dominio) {
            res.writeHead(503, { 'Content-Type': 'text/plain; charset=utf-8' });
            res.end('❌ No hay dominio disponible para generar el preview.');
        ...
        const imagenUrl = `https://${dominio}/menu18-banner.jpg`;
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
        res.end(`<!DOCTYPE html><html><head><meta charset="utf-8">
<meta property="og:title" content="🔞 NEXUS•NSFW">
<meta property="og:description" content="Made with ❤️ by Nexus•System">
<meta property="og:image" content="${imagenUrl}">
<meta property="og:type" content="website">
<meta name="twitter:card" content="summary_large_image">
<meta name="twitter:image" content="${imagenUrl}">
<title>🔞 NEXUS•NSFW</title>
</head><body><img src="${imagenUrl}" alt="Nexus NSFW"></body></html>`);
        return;
    }

    // ── Sirve el banner real que referencia la página de arriba ─────────────
    if (url === '/menu18-banner.jpg') {
        try {
            const { leerNsfwMenuMedia } = require('./src/menu');
            const media = leerNsfwMenuMedia();
            if (media && media.tipo === 'image' && fs.existsSync(media.path)) {
                const buf = fs.readFileSync(media.path);
                res.writeHead(200, { 'Content-Type': 'image/jpeg', 'Content-Length': buf.length });
                res.end(buf);
            ...
            console.error('Error sirviendo menu18-banner:', e.message);
        }
        res.writeHead(404, { 'Content-Type': 'text/plain' });
        res.end('No hay imagen de menú NSFW configurada');
        return;
    }
